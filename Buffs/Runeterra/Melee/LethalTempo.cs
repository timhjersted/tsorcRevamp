        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.buffTime[buffIndex] == 1)
            {
                if (modPlayer.BotCLethalTempoStacks > 1)
                {
                    ...
                    SoundEngine.PlaySound(new SoundStyle("tsorcRevamp/Sounds/Runeterra/Melee/LethalTempoFallOff") with { Volume = ModContent.GetInstance<tsorcRevampConfig>().BotCMechanicsVolume * 0.06f }, player.Center);
                }
                else
                {
                    modPlayer.BotCLethalTempoStacks = 0;
                    SoundEngine.PlaySound(new SoundStyle("tsorcRevamp/Sounds/Runeterra/Melee/LethalTempoFallOff") with { Volume = ModContent.GetInstance<tsorcRevampConfig>().BotCMechanicsVolume * 0.12f }, player.Center);
                }
            }
        }
