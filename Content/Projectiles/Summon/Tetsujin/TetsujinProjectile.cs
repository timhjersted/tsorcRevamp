...
namespace tsorcRevamp.Content.Projectiles.Summon.Tetsujin
{
    ...
    public class TetsujinProjectile : ModProjectile
    {
        ...
        private void Attack()
        {
            if (target != null && target.active && target.Distance(Projectile.Center) < 1000 && Main.myPlayer == Projectile.owner)
            {
                ...
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, projVel, ModContent.ProjectileType<FriendlyRedLaser>(), Projectile.damage * 4, 0, Main.myPlayer, target.whoAmI, Projectile.whoAmI);
                }

                if (Main.GameUpdateCount % 240 >= 180 && Main.GameUpdateCount % 240 <= 220 && Main.GameUpdateCount % 3 == 0)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, target.velocity + projVel * 10, ModContent.ProjectileType<FriendlyDragonsBreath>(), Projectile.damage / 4, 0, Main.myPlayer, target.whoAmI, Projectile.whoAmI);
                }

                if (Main.GameUpdateCount % 60 < 15 && Main.GameUpdateCount % 4 == 0)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, target.velocity + projVel * 17, ModContent.ProjectileType<FriendlyTetsujinLaser>(), Projectile.damage, 0, Main.myPlayer, target.whoAmI, Projectile.whoAmI);
                }
            }
        }
        ...
        private bool CheckActive(Player owner)
        {
            ...
            {
                owner.ClearBuff(ModContent.BuffType<TetsujinBuff>());

                return false;
            }

            if (owner.HasBuff(ModContent.BuffType<TetsujinBuff>()))
            {
        ...
        private void SearchForTargets(Player owner, out bool foundTarget, out float distanceFromTarget)
        {
            ...
            float angle = 0;
            if (owner.ownedProjectileCounts[ModContent.ProjectileType<TetsujinProjectile>()] > 0)
            {
                angle = (Main.GameUpdateCount / 120f) + (int)(Main.GameUpdateCount / 480f) + MathHelper.TwoPi * (Projectile.ai[0] / owner.ownedProjectileCounts[ModContent.ProjectileType<TetsujinProjectile>()]);
