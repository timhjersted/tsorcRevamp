...
namespace tsorcRevamp.Content.Projectiles.Summon.TripleThreat
{
    // This minion shows a few mandatory things that make it behave properly.
    // Its attack pattern is simple: If an enemy is in range of 43 tiles, it will fly to it and deal contact damage
    // If the player targets a certain NPC with right-click, it will fly through tiles to it
    // If it isn't attacking, it will float near the player with minimal movement
    public class FriendlyCataluminance : ModProjectile
    {
        ...
        private void Attack()
        {
            if (target != null && target.active && target.Distance(Projectile.Center) < 1000)
            {
                ...
                if (Main.GameUpdateCount % 240 == 180 && Main.myPlayer == Projectile.owner)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, projVel, ModContent.ProjectileType<FriendlyCatLightOfDawn>(), Projectile.damage * 2, 0, Main.myPlayer, 0, Projectile.whoAmI);
                }
            }
        }
        ...
        private bool CheckActive(Player owner)
        {
            ...
            {
                owner.ClearBuff(ModContent.BuffType<TripleThreatBuff>());

                return false;
            }

            if (owner.HasBuff(ModContent.BuffType<TripleThreatBuff>()))
            {
