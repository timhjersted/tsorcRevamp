...
namespace tsorcRevamp.Content.Projectiles.Enemy.GravelordNito
{
    ...
    ///<summary>
    ///MIASMA HEM: one of the two fixed poison-fog walls Nito plants ~25 tiles either side of the
    ///arena's spawn point (GravelordNito.RunMiasmaHem / SpawnMiasmaHem) to punish retreating to the
    ///edges instead of fighting near him. Built from HemPuffCount overlapping copies of the same
    ///NitoMiasmaVolume shader NitoMiasmaCloud uses, all drawn from this ONE projectile instead of
    ///HemPuffCount separate ones. Each puff's grid position, drift and fade phase are derived purely
    ///from its own index via a cheap hash, so nothing needs storing or syncing per puff — every
    ///machine draws the identical animation from Main.GlobalTimeWrappedHourly alone.
    ///</summary>
    class NitoMiasmaWall : ModProjectile
    {
        public override string Texture => UsefulFunctions.RefactorableFilepath(typeof(InvisibleNothingProj));

        internal const float WidthPixels = 144f;  // 9 tiles
        internal const float HeightPixels = 48f;  // 3 tiles
        const int HemPuffCount = 25;
        const int GridColumns = 5;
        const int GridRows = 5;               // GridColumns * GridRows must equal HemPuffCount

        const int FadeInTicks = 60;   // 1s materialising in
        const int HoldTicks = 1140;   // 19s fully up
        const int FadeOutTicks = 60;  // 1s dissolving — total lifetime 21s
        int Timer => (int)Projectile.localAI[0];

        public override void SetDefaults()
        {
            Projectile.hostile = true;
            Projectile.width = (int)WidthPixels;
            Projectile.height = (int)HeightPixels;
            Projectile.tileCollide = false;
            Projectile.penetrate = -1;
            Projectile.timeLeft = FadeInTicks + HoldTicks + FadeOutTicks;
            Projectile.aiStyle = 0;
        }

        // Damaging once it's substantially visible, off again the instant the final dissolve starts —
        // never an invisible hitbox at spawn, never a lingering one after the fog has visibly cleared.
        public override bool? CanDamage() => Timer >= FadeInTicks / 2 && Projectile.timeLeft > FadeOutTicks;

        public override void AI()
        {
            Projectile.localAI[0]++;
            Projectile.velocity = Vector2.Zero;
            Lighting.AddLight(Projectile.Center, 0.07f, 0.13f, 0.05f);
        }
        ...
        // Overall fade-in / hold / fade-out envelope; each puff's own breathing rides on top of this.
        float Envelope()
        {
            if (Timer < FadeInTicks)
            {
                return Timer / (float)FadeInTicks;
            }
            if (Projectile.timeLeft <= FadeOutTicks)
            {
                return Projectile.timeLeft / (float)FadeOutTicks;
            }
            return 1f;
        }
        ...
        {
            float value = (float)(System.Math.Sin(index * 12.9898) * 43758.5453);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 boxCenter = Projectile.Center;
            float time = Main.GlobalTimeWrappedHourly;

            for (int i = 0; i < HemPuffCount; i++)
            {
                int column = i % GridColumns;
                int row = i / GridColumns;
                float hash = HashToUnit(i);

                // A loose 5x5 grid spanning the wall, each puff nudged off-grid so it doesn't read as a
                // rigid lattice, plus a bit of overscan (0.42 not 0.5) so edge puffs still overlap their
                // neighbours instead of clipping the wall's own boundary.
                float baseX = MathHelper.Lerp(-WidthPixels * 0.42f, WidthPixels * 0.42f, column / (float)(GridColumns - 1))
                    + (hash - 0.5f) * 22f;
                float baseY = MathHelper.Lerp(-HeightPixels * 0.4f, HeightPixels * 0.4f, row / (float)(GridRows - 1))
                    + (HashToUnit(i + 97) - 0.5f) * 10f;

                // Drift: each puff wanders left/right over roughly 1.5-4 tiles on its own period, so the
                // whole wall never pulses as one rigid block. Vertical bob is smaller — a hem, not a cloud.
                float driftPeriod = 4.5f + hash * 3f;       // 4.5-7.5s per puff
                float driftAmplitude = 24f + hash * 40f;    // 1.5-4 tiles
                float drift = (float)System.Math.Sin(time / driftPeriod * MathHelper.TwoPi + i) * driftAmplitude;
                float bob = (float)System.Math.Sin(time / (driftPeriod * 0.6f) + i * 1.7f) * 6f;

                // Each puff fades in and out on its own cycle instead of all breathing in unison.
                float fadePeriod = 2.2f + hash * 2.2f;
                float puffFade = 0.35f + 0.55f * (0.5f + 0.5f * (float)System.Math.Sin(time / fadePeriod * MathHelper.TwoPi + i * 2.3f));

                float shaderProgress = (time * 0.12f + hash) % 1f;
                float puffScale = 0.55f + hash * 0.25f;
                Vector2 puffCenter = boxCenter + new Vector2(baseX + drift, baseY + bob);
                float puffRotation = (float)System.Math.Sin(time * 0.3f + i) * 0.15f;

                NitoVFX.DrawMiasma(puffCenter, new Vector2(50f, 46f) * puffScale, puffRotation,
                    shaderProgress, puffFade * envelope, i * 0.41f);
            }
