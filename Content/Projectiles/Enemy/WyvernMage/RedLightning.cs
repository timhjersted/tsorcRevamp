...
namespace tsorcRevamp.Content.Projectiles.Enemy.WyvernMage
{

    public class RedLightning : GenericLaser
    {
        ...
        public override void SetDefaults()
        {
            ...
            LaserTexture = TransparentTextureHandler.TransparentTextureType.RedLightning;
        ...
        private void SetLaserPosition()
        {
            if (!positionSet)
            {
                ...
                Tuple<List<Vector2>, List<float>, List<float>> initialLine = GenerateRedLightningLine(Projectile.position, Projectile.velocity.ToRotation(), segmentCount, false);
                    ...
                    if (branches[i].Count > 0)
                    {
                        for (int j = 0; j < branches[i].Count - 1; j++)
                        {
                            if (Main.rand.NextBool(3) && j > 5)
                            {
                                ...
                                Tuple<List<Vector2>, List<float>, List<float>> newBranch = GenerateRedLightningLine(branches[i][j], Projectile.velocity.ToRotation(), Main.rand.Next(segmentLimit), true);
        ...
        public Tuple<List<Vector2>, List<float>, List<float>> GenerateRedLightningLine(Vector2 initialPoint, float initialAngle, int maxLength, bool branch)
        {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
                lightningEffect = ModContent.Request<Effect>("tsorcRevamp/Effects/RedLightningLine", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
            }

            lightningEffect.Parameters["shaderColor"].SetValue(Color.Red.ToVector3());
            if (IsAtMaxCharge)
            {
        ...
        {
            GraphicsDevice device = Main.graphics.GraphicsDevice;
            device.Textures[0] = TransparentTextureHandler.TransparentTextures[TransparentTextureHandler.TransparentTextureType.RedLightning];
