...
namespace tsorcRevamp.Content.Projectiles.Enemy.WyvernMage
{
    ...
    public class SmallRedLightning : GenericLaser
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft - 120 > 0)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    Dust.NewDustPerfect(dustPos, DustID.FireworkFountain_Red, dustVel, 200, default, 0.8f).noGravity = true;
                }
            }
                            ...
                            if (FastContainsPoint(screenRect, branches[i][0]) || i == 0)
                            {
                                for (int j = 0; j < branches[i].Count - 1; j++)
                                {
                                    ...
                                    for (int k = 0; k < dustCount; k++)
                                    {
                                        ...
                                        thisDust.color = Color.Lerp(Color.White, Color.Red, lerpPercent);
                                    }
                                }
                            }
                        }
