...
namespace tsorcRevamp.Content.Items.Weapons.Summon.Whips.ModdedWhip
{
    public abstract class ModdedWhipProjectile : ModProjectile
    {
        ...
        {
            var globalProj = Projectile.GetGlobalProjectile<ModdedWhipGlobalProjectile>();
            ...
            globalProj.IsModded = true;
            if (MaxChargeTime > 0)
            {
                globalProj.CanBeCharged = true;
            }
            TimesHitThisSwing = 0;
        }
        ...
        public override void AI()
        {
            ...
            if (goredrinkerPlayer.Equipped && !player.HasBuff(ModContent.BuffType<GoredrinkerCooldown>()) && goredrinkerPlayer.Ready && Timer == 1) //this check is needed at the start to allow all goredrinker hits to calculate dmg correctly
            {
                goredrinkerPlayer.Swung = true;
            }
            ...
            if (Timer == swingTime / 2)
            {
                ...
                {
                    SoundEngine.PlaySound(new SoundStyle("tsorcRevamp/Sounds/Item/SummonerWhipcrack") with { Volume = Volume, PitchVariance = Pitch }, points[points.Count - 1]);
                }
            }
        }
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            Vector2 WhipTip = WhipTipBase * Main.player[Main.myPlayer].whipRangeMultiplier * Projectile.WhipSettings.RangeMultiplier * player.GetModPlayer<WhipTipHit>().WhipTipHitboxSize;
            List<Vector2> points = Projectile.WhipPointsForCollision;
            if (MaxChargeTime > 0)
            {
