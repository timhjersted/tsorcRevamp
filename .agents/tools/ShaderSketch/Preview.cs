...
//     cd .agents/tools/ShaderSketch && dotnet run
//
...
static class Preview
{
    ...
    // sample the exact game textures. The default preserves the normal in-place workflow: run from
    // .agents/tools/ShaderSketch/, three levels below the repo root. Every path below is relative to
    // that, so moving this folder means changing the depth HERE and in ContactSheet.ps1 — nowhere else.
    ...
    static void LoadTextures()
    {
        ...
        UnblockableSpriteSampler = new Tex(Path.Combine(ProjectRoot, "Projectiles", "Enemy", "BlackThrowingSpear"), previewLocal: true);
    ...
    static void Main()
    {
        ...
        // Renders go to the private tsorcDocs repo, not beside this file. The timestamped name means no
        // render ever overwrites another, so saving into the tracked preview/ folder grew the public repo
        // by ~82 MB in a month. PREVIEW_OUTPUT_DIR still overrides it; ProjectRoot resolves the same way
        // every texture path in this file does (relative to preview/, or PREVIEW_PROJECT_ROOT).
        string outputDirectory = Environment.GetEnvironmentVariable("PREVIEW_OUTPUT_DIR");

        if (string.IsNullOrWhiteSpace(outputDirectory))
        {
            outputDirectory = Path.Combine(ProjectRoot, "tsorcDocs", "VFXPreviews");
        }

        Directory.CreateDirectory(outputDirectory);
        string outputPath = Path.GetFullPath(Path.Combine(outputDirectory, outputName));
