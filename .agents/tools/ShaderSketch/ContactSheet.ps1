...
# This script lives three levels below the repo root (.agents\tools\ShaderSketch). The sheet is saved
# to the private tsorcDocs repo like every other preview image, never beside the script: this folder
# is tracked in the public repo, and tModLoader only skips paths starting with a dot.
$repoRoot = Join-Path $PSScriptRoot "..\..\.."
$root = Join-Path $repoRoot "Textures\Noise"
$outDir = Join-Path $repoRoot "tsorcDocs\VFXPreviews"
...
$outPath = [System.IO.Path]::GetFullPath((Join-Path $outDir $Out))
$sheet.Save($outPath)
Write-Output "wrote $outPath ($($Names.Count) textures, $(if ($Tile2x2) {'2x2 tiled'} else {'single tile'}))"
