# ShaderSketch — hand-ported shader preview
...
```powershell
cd .agents/tools/ShaderSketch
$env:PREVIEW_NAME = 'GigasSunPillar'; dotnet run  # -> tsorcDocs/VFXPreviews/GigasSunPillar-YYYYMMDD-HHMMSS.png
```
...
That two-background split is the point. See §42–§43 of the `vfx-shader-tips` skill (`../../skills/vfx-shader-tips/SKILL.md`) for why.
...
## Where this sits among the three harnesses

This is the **hand-ported** one: the HLSL maths is re-typed in C#, so it drifts from the shipped
shader within a few edits. For "does this match the game", use `../ShaderPreview/`, which runs the
real compiled `.xnb` on a GPU. Reach for this one when you are sketching maths that is **not in an
`.fx` yet** — that is the one thing the real renderer cannot do.

Output goes to `tsorcDocs/VFXPreviews/` (override with `PREVIEW_OUTPUT_DIR`), never into this folder:
it is tracked in the public repo. Renders from before 2026-09-10 are in `tsorcDocs/VFXPreviews/Archive/`.
