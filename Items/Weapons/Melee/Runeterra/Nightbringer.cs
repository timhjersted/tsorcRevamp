    ...
    public class Nightbringer: ModItem
    {
        ...
        public override void HoldItem(Player player)
        {
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if (other.active & !other.friendly & other.Distance(Main.MouseWorld) <= 25 & other.Distance(player.Center) <= 10000 & (player.GetModPlayer<tsorcRevampPlayer>().DoubleCritChance) & !player.HasBuff(ModContent.BuffType<NightbringerDashCooldown>()))
                {
                    if (DashingTimer > 0)
                    {
