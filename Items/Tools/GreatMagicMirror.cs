...
namespace tsorcRevamp.Items.Tools
{
    public class GreatMagicMirror : ModItem
    {
        ...
        public static bool checkWarpLocation(int x, int y)
        {
            ...
            {
                Main.NewText(LanguageUtils.GetTextValue("Items.GreatMagicMirror.OutOfBounds"), 255, 240, 20);
                return false;
            }
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            if (player.GetModPlayer<tsorcRevampPlayer>().BearerOfTheCurse)
            {
                ...
                {
                    Main.NewText(LanguageUtils.GetTextValue("Items.GreatMagicMirror.BotCDisabled"), Color.OrangeRed);
                }
                return false;
            }
            if (tsorcRevampWorld.BossAlive)
            {
                if (player.whoAmI == Main.myPlayer)
                {
                    Main.NewText(LanguageUtils.GetTextValue("CommonItemTooltip.UnusableDuringBoss"), Color.Yellow);
                }
                return false;
            }
            if (!player.GetModPlayer<tsorcRevampPlayer>().warpSet)
            {
                if (player.whoAmI == Main.myPlayer)
                {
                    Main.NewText(LanguageUtils.GetTextValue("Items.GreatMagicMirror.NoLocation"), 255, 240, 20);
                }
                return false;
            }
            else if (player.GetModPlayer<tsorcRevampPlayer>().warpWorld != Main.worldID)
            {
                if (player.whoAmI == Main.myPlayer)
                {
                    Main.NewText(LanguageUtils.GetTextValue("Items.GreatMagicMirror.WrongWorld"), 255, 240, 20);
                }
                return false;
            }
        ...
        public override void UseStyle(Player player, Rectangle rectangle)
        {
            ...
            if (checkWarpLocation(player.GetModPlayer<tsorcRevampPlayer>().warpX, player.GetModPlayer<tsorcRevampPlayer>().warpY))
            {
                ...
                {
                    Main.NewText(LanguageUtils.GetTextValue("Items.GreatMagicMirror.OnUse"), 255, 240, 20);
            ...
            {
                Main.NewText(LanguageUtils.GetTextValue("Items.GreatMagicMirror.Oops"), 255, 240, 20);
            }
            ...
            if (player.GetModPlayer<tsorcRevampPlayer>().BearerOfTheCurse)
            {
                ...
                {// if we find one
                 //insert the extra tooltip line
                    ...
                    tooltips.Insert(ttindex + 1, new TooltipLine(Mod, "BotCNoGreaterMM", LanguageUtils.GetTextValue("Items.GreatMagicMirror.BotCDisabled")));
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            ...
            if (!player.GetModPlayer<tsorcRevampPlayer>().warpSet)
            {
                ...
                Main.NewText(LanguageUtils.GetTextValue("Items.GreatMagicMirror.NewLocation"), 255, 240, 30);
            }
            else
            {
