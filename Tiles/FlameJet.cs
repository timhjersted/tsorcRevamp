        ...
        public override void NearbyEffects(int i, int j, bool closer)
        {
            if (!Main.gamePaused)
            {
                if (countdown <= 0)
                {
                    ...
                    Projectile.NewProjectile(new EntitySource_Misc("Flame Jet"), new Vector2((i * 16) + 8, (j * 16) + 8 - ((projSize * 16) / 2)), Vector2.Zero, ModContent.ProjectileType<Content.Projectiles.FlameJet>(), flameJetDamage, 0, Main.myPlayer, 0, projSize);
                    countdown = 3;
                }
                else
                {
