...
{
    public class SoulSkellyL : ModTile
    {
            ...
            if (!Main.gamePaused && Main.instance.IsActive && (!Lighting.UpdateEveryFrame || Main.rand.NextBool(4)))
            {
                ...
                if (Main.rand.NextBool(2) && tile.TileFrameX == 0) //If 0 is changed to 36, you end up with the dust duplicated 2 tiles past the intended location
                {
                    ...
                    if (frameY / 18 % 2 == 0)
                    {
                        ...
                        if (dustChoice != -1)
                        {
                            int dust = Dust.NewDust(new Vector2(i * 16 + 22, j * 16 - 1), 4, 4, dustChoice, 0f, 0f, 100, default(Color), 1f);
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if (tile.TileFrameX / 54 == 0)
            {
                ...
                Main.spriteBatch.Draw((Texture2D)Mod.Assets.Request<Texture2D>("Tiles/SoulSkellyL_Glow"), new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, height), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0);
            }
        }
        public override void MouseOver(int i, int j)
        {
            ...
            if (tile.TileFrameX / 54 == 0)
            {
                ...
                player.cursorItemIconID = ModContent.ItemType<SoulSkellyItemL>();
            }
        }
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (tile.TileFrameX / 54 == 0)
            {
                ...
                int y = j - Main.tile[i, j].TileFrameY / 18 % 1; // 1 because it is 1 blocks tall
