...
namespace tsorcRevamp.NPCs
{
    ...
    public static class SmartFighter4AI
    {
            ...
            float brakingPower = 0.2f)
        {
            ...
            if (npc.noTileCollide && !nav.PlatformPassActive && !npc.noGravity)
            {
                npc.noTileCollide = false;
            }
            ...
            if (!grounded && nav.AirCommitTimer > 0 && nav.AirCommitDirX != 0)
            {
                ...
                else
                {
                    ...
                    npc.velocity.X = MathHelper.Clamp(targetVx, -topSpeed * 1.3f, topSpeed * 1.3f);
                }
            }
                ...
                && navigationRequiresHold)
            {
                ...
                npc.direction = holdDir;
                npc.spriteDirection = holdDir;
                actionHandled = true;
                actionLabel = "hold-fire";
                reasonLabel = holdReason;
            }
            ...
            else
            {
                ...
                if (!kiting && nav.Plan != null && nav.PlanIndex < nav.Plan.Count)
                {
                    ...
                    {
                        globalNPC.BeastUnreachableFrames = 0;  // reachable: A* is navigating us in to touch
                    }
                        ...
                        else
                        {
                            ...
                            if (!actionHandled)
                            {
                                ...
                                if (allowCliffDrop)
                                {
                                    ...
                                    actionLabel = "chase-drop";
                                    reasonLabel = "player-below-ledge";
                                }
                                else if (xAlignedDiffLevel)
                                {
                                    ...
                                    actionLabel = "hold-aligned";
                                    reasonLabel = "under/over-player,no-path";
                                }
                                else
                                {
                ...
                if (execMove)
                {
                    if (Math.Abs(npc.Center.X - nav.MoveCheckX) > 2f)
                    {
                        nav.MoveCheckX = npc.Center.X;
                    ...
                    && (nav.Plan != null || actionLabel == "blocked"))
                {
                    ...
                    {
                        nav.HardStuckCheckX = npc.Center.X;  // open a fresh window
                    }
                    if (++nav.HardStuckFrames >= 120) // ~2s window elapsed
                    {
                        if (Math.Abs(npc.Center.X - nav.HardStuckCheckX) > 48f)
                        {
                            nav.HardStuckStrikes = 0;  // made real ground
                        }
                        else
                        {
                            nav.HardStuckStrikes++;  // confined this window
                        }
        ...
        private static void TickTimers(NavState nav)
        {
            ...
            if (nav.RepositionTimer > 0)
            {
                nav.RepositionTimer--;
            }
            if (nav.PlanGraceFrames > 0)
            {
                nav.PlanGraceFrames--;
            }
        }
        ...
        private static bool ShouldReplan(NavState nav, NPC npc, Player player)
        {
            ...
            if (Math.Abs(player.Center.X - planEndX) > 16 * TileF)
            {
                return true;
            }
            if (Math.Abs(player.Center.Y - planEndY) > 10 * TileF)
            {
                return true;
            }
            return false;
            ...
            foreach (var a in spans)
            {
                ...
                for (int dy = -1; dy <= 1; dy++)
                {
                    ...
                    foreach (var b in bucket)
                    {
                        ...
                        if (dy == -1 && !IsNavigationSolid(joinX, a.Y))
                        {
                            continue;
                        }
                ...
                foreach (var b in spans)
                {
                    ...
                    if (dy > MaxPlanRiseTiles() || dy < -8)
                    {
                        continue;  // outside this NPC's jump reach
                    }
                ...
                if (!_canUseRopes)
                {
                    continue;  // ropes are a per-enemy opt-in (CanUseRopes); skip building rope edges
                }
                for (int x = a.LeftX - 1; x <= a.RightX + 1; x++)
                {
                    ...
                    if (ropeTopY >= a.Y)
                    {
                        continue;  // rope must extend above this span to climb
                    }
                    foreach (var landingSpan in spans)
                    {
                        ...
                        {
                            continue;  // rope must sit under/adjacent the landing span
                        }
        ...
        private static bool FindRopeSpan(int x, int feetY, int reach, out int bottomY, out int topY)
        {
            ...
            {
                return false;  // need at least 3 contiguous rope tiles
            }
            bottomY = bottomRow;
            topY = topRow;
            return true;
        ...
        private static bool RopeSideExitClear(int ropeCol, Span landingSpan)
        {
            ...
            if (sideX == ropeCol)
            {
                return false;  // landingSpan sits over the rope column — not a sideways exit
            }
        ...
        private static bool RopeTopJumpClear(int ropeCol, int ropeTopY, Span landingSpan)
        {
            for (int y = ropeTopY - 1; y >= landingSpan.Y; y--)
            {
                ...
                {
                    return false;  // solid in the way — can't jump through it
                }
            }
            if (!IsStandableTile(ropeCol, landingSpan.Y + 1))
            {
                return false;  // must actually land on the platform above the rope
            }
        ...
        private static bool TryGrabNearbyRope(NavState nav, NPC npc, Player player)
        {
            if (!_canUseRopes)
            {
                return false;  // ropes are a per-enemy opt-in (CanUseRopes)
            }
            if (nav.Plan != null)
            {
                return false;  // only as a no-plan fallback
            }
            int now = (int)Main.GameUpdateCount;
            int npcCx = (int)(npc.Center.X / TileF);
            int npcFeetY = GetFeetTileY(npc);
            ...
            if (Math.Abs(vdelta) < 3)
            {
                nav.RopeFallbackFails = 0;
                nav.LastPlanResult = "rope-skip same-level";
                return false;
            }
            bool goUp = vdelta < 0;
            ...
            {
                int x = npcCx + dx;
                if (!FindRopeSpan(x, npcFeetY, 5, out int ropeBottomY, out int ropeTopY))
                {
                    continue;
                }
                // Rope must extend in the direction of the player.
                if (goUp && ropeTopY >= npcFeetY - 1)
                {
                    continue;  // no rope above us
                }
                if (!goUp && ropeBottomY <= npcFeetY + 1)
                {
                    continue;  // no rope below us
                }
                ...
                if (IsRopeFallbackBadEdged(nav, x, tY, now))
                {
                    continue;
                }
            ...
            {
                nav.LastPlanResult = $"rope-none up={goUp} d={vdelta}";
                return false;
            }
        ...
        {
            foreach (var kv in nav.BadEdgeTargets)
                if (kv.Value > now && Math.Abs(kv.Key.x - x) <= 1 && Math.Abs(kv.Key.y - y) <= 2)
                {
                    return true;
                }
            return false;
            ...
            float topSpeed, float acceleration, bool los, out string action, out string reason)
        {
            ...
            if (Math.Abs(player.Center.Y - npc.Center.Y) > 48f)
            {
                return false;
            }

            float distTiles = npc.Distance(player.Center) / TileF;
            if (distTiles > globalNPC.KiteRangeMax)
            {
                return false;  // too far → let pursuit close in (don't take control)
            }
            ...
            if (distTiles < nav.KiteTargetDist - deadband && !nav.KiteLetClose)
            {
                ...
                if (IsCliffAhead(npc, away) || GetObstacleHeight(fX, GetFeetTileY(npc)) > 1)
                {
                    ...
                    action = "kite-hold";
                    reason = $"d={distTiles:F0} cornered";
                }
                else
                {
        ...
        private static int BeastStepDir(NPC npc, int desired, int faceP)
        {
            ...
            if (!IsBeastStepBlocked(npc, desired))
            {
                return desired;
            }
            if (!IsBeastStepBlocked(npc, -desired))
            ...
            float acceleration, out string action, out string reason)
        {
            ...
            if (touching)
            {
                nav.BeastContactBackoff = globalNPC.BeastContactBackoffTicks;
            }
            if (nav.BeastContactBackoff <= 0)
            {
                return false;
            }
            ...
            npc.direction = faceP;
            npc.spriteDirection = faceP; // keep facing the player while bobbing out
            action = "beast-backoff";
            reason = $"t={nav.BeastContactBackoff}";
            return true;
        }
        ...
        private static bool TryBeastPhase(NavState nav, NPC npc, tsorcRevampGlobalNPC globalNPC)
        {
            if (nav.BeastPhasing)
            {
                ...
                if (Math.Abs(npc.velocity.X) < 1f || Math.Sign(npc.velocity.X) != dir)
                {
            ...
            if (dist <= 0)
            {
                return false;  // body isn't actually blocked ahead
            }
            if (dist > globalNPC.BeastPhaseMaxWidth)
            {
                return false;  // a real wall → let A* handle it
            }
            ...
            if (!IsNavigationSolid(farX, feetY + 1) && !IsNavigationSolid(farX, feetY + 2))
            {
                return false;
            }
        ...
        private static void ApplyBeastGroundSink(NPC npc, tsorcRevampGlobalNPC globalNPC)
        {
            ...
            for (int x = leftX; x <= rightX; x++)
            {
                ...
                if (!IsNavigationSolid(x, restRow) && !IsNavigationSolid(x, restRow - 1))
                {
                    continue;
                }

                for (int sinkDepth = 0; sinkDepth <= sinkCap; sinkDepth++)
                {
                    if (IsNavigationSolid(x, restRow + sinkDepth))
                    {
                        if (sinkDepth > deepest)
                        {
                            deepest = sinkDepth;
                        }
                        break;
            ...
            if (sinkPixels > npc.gfxOffY)
            {
                npc.gfxOffY = sinkPixels;  // sink down to follow the ground; never reduce a step-up glide
            }
        }
        ...
        {
            if (npc.velocity.Y != 0f || Math.Abs(npc.velocity.X) > 0.6f)
            {
                return;
            }
            ...
            if (leftAir == rightAir)
            {
                return;  // both float (narrow perch) or both grounded → no clearly better side
            }
        ...
        private static bool BeastSideOverhangsAir(NPC npc, int side, int sinkCap)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                for (int belowOffset = 1; belowOffset <= sinkCap + 1; belowOffset++)
                    if (IsNavigationSolid(x, feetY + belowOffset))
                    {
                        return false;  // ground close below this column → not floating
                    }
            }
            return true;
        }
            ...
            float topSpeed, float acceleration, bool los, bool inRange, out string action, out string reason)
        {
            ...
            if (!(globalNPC.AttackList.Count > 0 && inRange && playerAbove))
            {
                return false;
            }
        ...
        private static int StandoffSideScore(NPC npc, Player player, int dir)
        {
            ...
            int reachable = 0;
            for (int stepOut = 1; stepOut <= StandoffMinTiles; stepOut++)
            {
                int x = startX + dir * stepOut;
                if (GetObstacleHeight(x, feetY) > 1)
                {
                    break;  // wall this way -> can't walk farther
                }
                if (GetDropDepth(x, feetY, 4) >= 4)
                {
                    break;  // cliff this way -> can't walk farther
                }
                reachable = stepOut;
                Vector2 spot = new Vector2(x * TileF + 8f, npc.Center.Y);
                if (HasStrictLineOfSight(spot, player.Center))
                    return 100 + (StandoffMinTiles - stepOut);       // LOS opens at stepOut tiles; nearer = higher score
            }
            return reachable;
        }
        ...
        private static Span FindContainingSpan(List<Span> spans, int x, int feetY)
        {
            ...
            foreach (var span in spans)
            {
                ...
                if (dx > 16 || dy > Math.Max(12, MaxPlanRiseTiles()))
                {
                    continue;
                }
        ...
        private static bool IsAligned(NPC npc, int launchX)
        {
            ...
            return Math.Abs(npc.Center.X - launchCenterX) <= AlignTolerancePx;
        }

        private static int AlignDir(NPC npc, int launchX)
        {
            float launchCenterX = launchX * TileF + 8f;
            return npc.Center.X < launchCenterX ? 1 : -1;
        }
            ...
            float jumpCeil, float boostCeil, bool grounded, out string action, out string reason)
        {
            ...
            if (grounded && !onRope && !closeEnoughToGrab && Math.Abs(npc.Center.X - ropeCenter) > 4f)
            {
                ...
                if (npc.collideX && obstacleHeight > 2)
                {
                ...
                action = "align-rope";
                reason = $"ropeX={step.LaunchX} cx={npc.Center.X / TileF:F1} {(descend ? "down" : "up")}";
                return true;
            }
            nav.AlignStallFrames = 0;
            ...
            if (onRope && (targetExitSafe || atRopeEnd || forceDismount || ceilingCapped))
            {
                ...
                if (!descend && !reachedTarget && IsNavigationSolid(step.LaunchX, feetY - 2))
                {
                    ...
                    nav.RopeEngaged = false;
                    action = "rope-deadend";
                    reason = $"toY={step.TargetY} ropeTop~{feetY} solidAbove";
                    return false;
                }
                ...
                if (forceDismount && nav.RopeJumpedThisStep)
                {
                    ...
                    nav.CommitFrames = 0;
                    action = "rope-abort";
                    reason = $"blocked toY={step.TargetY}";
                    return false;
                }
                ...
                if (!descend && (atRopeEnd || forceDismount) && rise >= 2 && !solidAbove)
                {
                    ...
                    if (forceDismount)
                    {
                        nav.RopeJumpedThisStep = true;
                    }
                    action = "rope-jumpup";
                    reason = $"rise={rise} p={Math.Max(dismountPower, 6f):F1}";
                    return true;
                }
                ...
                npc.direction = offDir;
                npc.spriteDirection = offDir;
                ...
                action = descend ? "rope-exit-down" : "rope-exit";
                reason = $"toY={step.TargetY} dir={offDir}";
                return true;
            }
            ...
            {
                action = "rope-approach";
                reason = $"cx={npc.Center.X / TileF:F1} ropeX={step.LaunchX}";
                return true;
            }
        ...
        {
            if (Math.Abs(step.TargetX - step.LaunchX) != 1)
            {
                return false;
            }
            ...
            out string action, out string reason)
        {
            ...
            if (doorDamage > 0 && TryDoor(npc, dir, doorDamage, globalNPC, out action, out reason))
            {
                return true;
            }
            ...
            float jumpCeil, float boostCeil, bool grounded, int feetY, out string action, out string reason)
        {
            ...
            if (nav.JumpFiredThisStep)
            {
                ...
                npc.velocity.X *= 0.5f;
                action = "jump-missed";
                reason = $"undershot target=({step.TargetX},{step.TargetY})";
                return true;
            }
            ...
            if (!aligned)
            {
                ...
                {
                    if (Math.Abs(npc.velocity.X) > 0.7f)
                    {
                        npc.velocity.X *= 0.6f;
                    }
                ...
                if (Math.Abs(npc.Center.X - nav.AlignLastX) > 1.5f)
                {
                    nav.AlignLastX = npc.Center.X;
                    nav.AlignStallFrames = 0;
                }
                else if (++nav.AlignStallFrames > 18)
                {
                    nav.StepTimer = 0;
                    nav.AlignStallFrames = 0;
                }
            ...
            if (rise < 0 && absDx <= 1)
            {
                ...
                reason = $"down rise={rise} plat={platformBelow}";
                return true;
            }
            ...
            float maxJumpPower, float maxLaunchVx, out float jumpPower, out float launchVx)
        {
            ...
            for (float power = startPower; power <= maxJumpPower + 1e-3f; power += 0.1f)
            {
                ...
                if (disc < 0f)
                {
                    continue;  // can't reach that height at this power
                }
                float airtime = (power + (float)Math.Sqrt(disc)) / gravity; // descending-branch landing
                if (airtime <= 0f)
                {
                    continue;
                }
        ...
        private static (float power, float boost) NearestArc(int absDx, int dy, float jumpCeil, float boostCeil)
        {
            ...
            (float power, float boost) best = (jumpCeil, boostCeil * 0.5f);
            foreach (var kv in JumpArcs)
            {
                int keyDist = Math.Abs(kv.Key.dx - absDx) + Math.Abs(kv.Key.dy - dy);
                if (keyDist < bestKeyDist)
                {
                    bestKeyDist = keyDist;
                    best = kv.Value;
                }
            ...
            bool grounded, out string action, out string reason)
        {
            ...
            int descend = step.TargetX > step.LaunchX ? 1
                        : step.TargetX < step.LaunchX ? -1
                        : npc.direction;
            npc.direction = descend;
            npc.spriteDirection = descend;
            ...
            action = "drop";
            reason = $"toY={step.TargetY} plat={platformBelow}";
            return true;
        }
        ...
        {
            int clearRows = 0;
            for (int rowsUp = 1; rowsUp <= 8; rowsUp++)
            {
                if (IsNavigationSolid(col, feetY - rowsUp))
                {
                    break;
                }
                clearRows = rowsUp;
        ...
        private static int BodyUpClear(NPC npc, int feetY)
        {
            ...
            for (int col = leftCol; col <= rightCol; col++)
                minClear = Math.Min(minClear, ColumnUpClear(col, feetY));
            return minClear;
        }
        ...
        private static bool TryShaftEscape(NavState nav, NPC npc, Player player, float jumpCeil, float topSpeed, float acceleration, out string action, out string reason)
        {
            action = "";
            reason = "";
            if (player.Center.Y >= npc.Center.Y - 2f * TileF)
            {
                return false;  // player not meaningfully above
            }
            ...
            if (curUp >= 4)
            {
                ...
                action = "shaft-escape";
                reason = $"up={curUp} drift={driftDir}";
                return true;
            }
            ...
            npc.direction = stepDir;
            npc.spriteDirection = stepDir;
            ApplyChase(npc, stepDir, topSpeed, Math.Max(acceleration, AlignAccelFloor));
            action = "shaft-align";
            reason = $"backup {stepDir} (under lip, curUp={curUp})";
            return true;
        }
            ...
            float boostCeil, float topSpeed, bool allowCliffDrop, out string action, out string reason)
        {
            ...
            if (obstacleHeight > 1)
            {
                ...
                if (!nav.NoAStarPath && obstacleHeight <= 6 && HasHeadroomForJump(npc, direction, obstacleHeight))
                {
            ...
            if (drop >= 2 && !allowCliffDrop)
            {
                ...
                if (gapFound && gap >= 1 && gap <= 7 && landDrop <= 2)
                {
                    ...
                    npc.velocity.X *= 0.5f;
                    action = "gap-halt";
                    reason = $"infeasible gap={gap},drop={landDrop} g={npc.gravity:F2}";
                    return true;
                }
            }
        ...
        private static void ApplyChase(NPC npc, int direction, float topSpeed, float acceleration)
        {
            ...
            if (npc.velocity.X < targetVx)
            {
                npc.velocity.X += acceleration;
                if (npc.velocity.X > targetVx)
                {
                    npc.velocity.X = targetVx;
                }
            }
            else if (npc.velocity.X > targetVx)
            {
                npc.velocity.X -= acceleration;
                if (npc.velocity.X < targetVx)
                {
        ...
        private static bool IsStandableTile(int x, int y)
        {
            ...
            if (!(IsNavigationSolid(x, y) || IsPlatformTile(x, y)))
            {
                return false;
            }
            ...
            if (_minSurfaceWidth > 0 && !UsefulFunctions.IsFootprintSupported(x, y, _minSurfaceWidth, _maxSurfaceStep))
            {
                return false;
            }
            return true;
        ...
        private static int GetObstacleHeight(int frontX, int feetY)
        {
            ...
            {
                return 0;  // open at foot level -> nothing to climb
            }
            int height = 1;
        ...
        private static bool LaunchTrailClear(int launchCol, int landX, int feetY)
        {
            int landDir = Math.Sign(landX - launchCol);
            const int rows = 3; // body footprint height in tiles
            for (int y = feetY; y > feetY - rows; y--)
            {
                if (landDir == 0)
                {
                    if (IsNavigationSolid(launchCol - 1, y) || IsNavigationSolid(launchCol + 1, y))
                    {
                        return false;
                    }
                }
                else if (IsNavigationSolid(launchCol - landDir, y))
                {
                    return false;  // trailing side = opposite the landing
                }
            }
            return true;
        }
        ...
        private static bool TryFindJumpEdge(Span a, Span b, int dy, out int launchX, out int landX, out int absDx)
        {
            ...
            foreach (int launchCol in launches)
            {
                ...
                else
                {
                    ...
                    if (dy > 0)
                    {
                        bool cleanVertical = IsPlatformTile(launchCol, b.Y + 1);
                        if (cleanVertical)
                        {
                            lands.Add(launchCol);  // straight-up through a platform first
                        }
                        ...
                        if (!cleanVertical)
                        {
                            lands.Add(launchCol);  // vertical only as last resort over solid
                        }
                    }
                    else
                    {
                        lands.Add(launchCol); // same column = pure vertical
                        if (launchCol + 1 <= oMax)
                        {
                            lands.Add(launchCol + 1);
                        }
                ...
                foreach (int landCol in lands)
                {
                    ...
                    if (dy >= 1 && adx <= 1 && !LaunchTrailClear(launchCol, landCol, a.Y))
                    {
                        continue;
                    }
        ...
        private static bool IsPlatformTile(int x, int y)
        {
            ...
            return TileID.Sets.Platforms[tile.TileType] || Main.tileSolidTop[tile.TileType];
        }
        ...
        private static bool IsNavigationSolid(int x, int y)
        {
            ...
            return !Main.tileFrameImportant[tile.TileType];
        }
            ...
            bool attack, string action, string reason)
        {
            ...
            if (now - nav.LastLogTick < interval)
            {
                return;
            }
            nav.LastLogTick = now;
            try
            {
                ...
                if (nav.Plan != null)
                {
                    ...
                    if (nav.PlanIndex < nav.Plan.Count)
                    {
                        var planStep = nav.Plan[nav.PlanIndex];
                        planStr += $" {planStep.Kind}->{planStep.TargetX},{planStep.TargetY}@launch{planStep.LaunchX}";
                    }
                }
                ...
                {
                    var ropeStep = nav.Plan[nav.PlanIndex];
                    int launchCol = ropeStep.LaunchX;
                    int feetY = GetFeetTileY(npc);
                    bool descend = nav.RopeDirLatched ? nav.RopeDescend : ropeStep.TargetY > feetY;
                    bool atEnd = nav.RopeEngaged && (descend ? !IsRopeTile(launchCol, feetY + 1) : !IsRopeTile(launchCol, feetY - 1));
                    bool reached = descend ? feetY >= ropeStep.TargetY : feetY <= ropeStep.TargetY;
                    ropeStr = $" [rope {(descend ? "DN" : "UP")} toY={ropeStep.TargetY} feetY={feetY} lastFeetY={nav.LastRopeFeetY}"
                        + $" ropeU1={IsRopeTile(launchCol, feetY - 1)} ropeHead={IsRopeTile(launchCol, feetY - 2)} ropeD1={IsRopeTile(launchCol, feetY + 1)}"
                        + $" solidU2={IsNavigationSolid(launchCol, feetY - 2)} solidU3={IsNavigationSolid(launchCol, feetY - 3)}"
