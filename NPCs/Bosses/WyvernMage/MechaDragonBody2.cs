		...
		public override void AI() {
			...
			if (!Main.npc[(int)npc.ai[1]].active)
			{
				...
				for (int num36 = 0; num36 < 50; num36++)
				{
					...
					int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y - 10), npc.width, npc.height, DustID.Fire, Main.rand.Next(-20, 20) * 2, Main.rand.Next(-20, 20) * 2, 100, color, 4f);
					Main.dust[dust].noGravity = true;
					dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y - 10), npc.width, npc.height, DustID.Fire, Main.rand.Next(-20, 20) * 2, Main.rand.Next(-20, 20) * 2, 100, color, 4f);
					Main.dust[dust].noGravity = true;
					dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y - 10), npc.width, npc.height, DustID.Fire, Main.rand.Next(-20, 20) * 2, Main.rand.Next(-20, 20) * 2, 100, color, 4f);
					Main.dust[dust].noGravity = true;
					dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y - 10), npc.width, npc.height, DustID.Fire, 0, 0, 100, Color.White, 2.0f);
					Main.dust[dust].noGravity = true;
				}
			...
			if (Timer >= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					...
					int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, (float)((Math.Cos(rotation) * num48) * -1), (float)((Math.Sin(rotation) * num48) * -1), ModContent.ProjectileType<Projectiles.Enemy.CrystalFire>(), 30, 0f, 0);
			...
			if (Main.rand.Next(2) == 0)
			{
				...
				if (ModContent.GetInstance<tsorcRevampConfig>().LegacyMode)
				{
					...
					dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, DustID.PurpleTorch, 0, 0, 100, Color.White, 2.0f);
				} else
                {
					dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, DustID.Fire, 0, 0, 100, Color.White, 2.0f);
				}
				Main.dust[dust].noGravity = true;
			}
		}
