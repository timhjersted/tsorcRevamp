		...
		public override void AI() {
			...
			if (Timer >= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					...
					{
						int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, (float)((Math.Cos(rotation) * num48) * -1), (float)((Math.Sin(rotation) * num48) * -1), ModContent.ProjectileType<Projectiles.Enemy.CrystalFire>(), CrystalFireDamage, 0f, Main.myPlayer);
					}
