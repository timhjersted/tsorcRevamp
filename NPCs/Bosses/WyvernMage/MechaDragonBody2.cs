        ...
        public override void AI()
        {
            ...
            if (Timer >= 480)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    {
                        int num54 = Projectile.NewProjectile(NPC.GetSource_FromThis(), vector8.X, vector8.Y, (float)((Math.Cos(rotation) * num48) * -1), (float)((Math.Sin(rotation) * num48) * -1), ModContent.ProjectileType<HostileFreezeBolt>() , HostileFreezeBoltDamage, 0f, Main.myPlayer);
                    }
                    Timer = 0;
                }
