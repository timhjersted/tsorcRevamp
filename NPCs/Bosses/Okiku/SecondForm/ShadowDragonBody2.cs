	...
	public class ShadowDragonBody2 : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (this.Timer >= 0 && Main.netMode != 2)
			{
				...
				Projectile.NewProjectile(vector8.X, vector8.Y, (float)(Math.Cos(rotation) * (double)num48 * -1.0), (float)(Math.Sin(rotation) * (double)num48 * -1.0), base.mod.GetProjectile("ObscureSaw").projectile.type, 50, 0f, Main.myPlayer);
