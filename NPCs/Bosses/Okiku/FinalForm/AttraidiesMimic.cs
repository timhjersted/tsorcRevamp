    ...
    class AttraidiesMimic : ModNPC {
        ...
        public override void AI() {
            ...
            if (attackPhase == 0) // PHASE 0
            {
                ...
                if (genericTimer2 == 15) {
                    if (left) {
                        Projectile.NewProjectile(vector8.X, vector8.Y, -6 + Main.rand.Next(-1, 1), Main.rand.Next(-10, 10) / 5, ModContent.ProjectileType<CrazedOrb>(), 56, 0f, 0);
                    }
                    else {
                        Projectile.NewProjectile(vector8.X, vector8.Y, 6 + Main.rand.Next(-1, 1), Main.rand.Next(-10, 10) / 5, ModContent.ProjectileType<CrazedOrb>(), 56, 0f, 0);
                    }
                ...
                if (genericTimer2 >= 50) {
                    ...
                    if (subPhase == 0) 
                    {
                        for (int num36 = 0; num36 < 9; num36++) {
                            Projectile.NewProjectile(vector8.X, vector8.Y, (float)Math.Sin(randomrot + ((360 / 13) * (1 + num36)) * 3), (float)Math.Cos(randomrot + ((360 / 13) * (1 + num36)) * 3), ModContent.ProjectileType<ShadowOrb>(), 50, 0f, 0);
                        }
                        genericTimer2 = 0;
                    }
                    if (subPhase == 1)
