...
namespace tsorcRevamp.NPCs.Bosses
{
    class HeroofLumelia : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            NPC.rarity = 4;
            despawnHandler = new NPCDespawnHandler(LaUtils.GetTextValue("NPCs.HeroofLumelia.DespawnHandler"), Color.Gold, DustID.GoldFlame);
        }
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            if (Main.hardMode && !tsorcRevampWorld.SuperHardMode && P.ZoneOverworldHeight && P.ZoneDesert && tsorcRevampWorld.NewSlain.ContainsKey(new NPCDefinition(ModContent.NPCType<NPCs.Bosses.HeroofLumelia>())) && !(P.ZoneCorrupt || P.ZoneCrimson) && !P.ZoneBeach && Main.rand.NextBool(500)) { UsefulFunctions.BroadcastText(LaUtils.GetTextValue("NPCs.HeroofLumelia.Spawn"), 175, 75, 255); return 1; }
        ...
        public override void AI()  //  warrior ai
        {
            ...
            if (shoot_and_walk && Main.netMode != 1 && !Main.player[NPC.target].dead) // can generalize this section to moving+projectile code
            {
                ...
                if (NPC.Distance(player.Center) > 130 && Main.rand.NextBool(350))
                {
                    ...
                    {
                        UsefulFunctions.BroadcastText(LaUtils.GetTextValue("NPCs.HeroofLumelia.Archers1"), 175, 75, 255);
                    }
                ...
                if (NPC.Distance(player.Center) > 800 && Main.rand.NextBool(100))
                {
                    ...
                    if (Main.rand.NextBool(4))
                    {
                        UsefulFunctions.BroadcastText(LaUtils.GetTextValue("NPCs.HeroofLumelia.Archers2"), 175, 75, 255);
                    }
        ...
        {
            target.AddBuff(ModContent.BuffType<GrappleMalfunction>(), 60 * 60, false); //1 minute
            target.AddBuff(ModContent.BuffType<Crippled>(), 20 * 60, false); // loss of flight mobility for 20 seconds, down from 4 minutes moohahaha... cough cough
