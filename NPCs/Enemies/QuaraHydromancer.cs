	...
	class QuaraHydromancer : ModNPC
	{
		...
		public override void AI()  //  warrior ai
		{
			...
			if (Main.netMode != 1 && can_teleport && npc.ai[3] >= (float)boredom_time) // is server & chaos ele & bored
			{
				...
				int tp_radius = 26; // radius around target(upper left corner) in blocks to teleport into
				int tp_counter = 0;
				bool flag7 = false;
				...
				while (!flag7) // loop always ran full 100 time before I added "flag7 = true" below
				{
					...
					{ // (tp_x_target,m) is block under its feet I think
						if ((m < target_y_blockpos - 16 || m > target_y_blockpos + 16 || tp_x_target < target_x_blockpos - 16 || tp_x_target > target_x_blockpos + 16) && (m < y_blockpos - 1 || m > y_blockpos + 1 || tp_x_target < x_blockpos - 1 || tp_x_target > x_blockpos + 1) && Main.tile[tp_x_target, m].active())
						{ // over 16 blocks distant from player & over 1 block distant from old position & tile active(to avoid surface? want to tp onto a block?)
						...
						} // END over 16 blocks distant from player...
