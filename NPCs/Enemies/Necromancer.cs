		...
		public override void AI()  //  warrior ai
		{
			...
			if (shoot_and_walk && Main.netMode != 1 && !Main.player[npc.target].dead) // can generalize this section to moving+projectile code
			{
				...
				if (customAi1 >= 10f)
				{
					...
					if ((customspawn1 < 11) && Main.rand.Next(200) == 1)
					{
						int Spawned = NPC.NewNPC((int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), NPCID.ArmoredSkeleton, 0);
			...
			if (Main.netMode != 1 && can_teleport && npc.ai[3] >= (float)boredom_time) // is server & chaos ele & bored
			{
				...
				while (!flag7) // loop always ran full 100 time before I added "flag7 = true" below
				{
					...
					{ // (tp_x_target,m) is block under its feet I think
						if ((m < target_y_blockpos - 6 || m > target_y_blockpos + 6 || tp_x_target < target_x_blockpos - 6 || tp_x_target > target_x_blockpos + 6) && (m < y_blockpos - 1 || m > y_blockpos + 1 || tp_x_target < x_blockpos - 10 || tp_x_target > x_blockpos + 10) && Main.tile[tp_x_target, m].active())
						{ // over 6 blocks distant from player & over 10 block distant from old position & tile active(to avoid surface? want to tp onto a block?)
