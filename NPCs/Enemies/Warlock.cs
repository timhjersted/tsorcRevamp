...
namespace tsorcRevamp.NPCs.Enemies
{
	class Warlock : ModNPC
	{
		...
		public override void ScaleExpertStats(int numPlayers, float bossLifeScale)
		{
			...
			beamBallDamage = (int)(beamBallDamage / 2);
		}

		int greatEnergyBeamDamage = 35;
		int beamBallDamage = 35;
		...
		public override void AI()  //  warrior ai
		{
			...
			float enrage_top_speed = 5.6f;  //  faster when enraged, usually 2*top_speed
				...
				if (customAi1 >= 10f)
				{
					...
					if (Main.rand.Next(50) == 1)
					{
						...
						if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
						{
							...
							speedX *= num51;
							speedY *= num51;
							int type = ModContent.ProjectileType<Projectiles.Enemy.EnemySpellGreatEnergyBall>();//44;//0x37; //14;
							int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, greatEnergyBeamDamage, 0f, Main.myPlayer);
					...
					if (Main.rand.Next(15) == 0) //1 in 25 chance boss will summon an NPC
					{
					...
					if (Main.rand.Next(100) == 1)
					{
						...
						if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
						{
							...
							int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, beamBallDamage, 0f, Main.myPlayer);
			...
			if (Main.netMode != 1 && can_teleport && npc.ai[3] >= (float)boredom_time) // is server & chaos ele & bored
			{
				...
				int tp_counter = 0;
				bool flag7 = false;
				if (Math.Abs(npc.position.X - Main.player[npc.target].position.X) + Math.Abs(npc.position.Y - Main.player[npc.target].position.Y) > 200000f)
				{ // far away from target; 2000 pixels = 125 blocks
					tp_counter = 100;
					flag7 = false; // no teleport -- was true, will make warlock always teleport
				}
				while (!flag7) // loop always ran full 100 time before I added "flag7 = true" below
				{
					...
					{ // (tp_x_target,m) is block under its feet I think
						if ((m < target_y_blockpos - 7 || m > target_y_blockpos + 7 || tp_x_target < target_x_blockpos - 7 || tp_x_target > target_x_blockpos + 7) && (m < y_blockpos - 1 || m > y_blockpos + 1 || tp_x_target < x_blockpos - 1 || tp_x_target > x_blockpos + 1) && Main.tile[tp_x_target, m].active())
