...
{
    // Pre-port backup of LothricSpearKnight (bespoke state-machine movement + hand-rolled nav).
    // Kept for A/B reference; the live LothricSpearKnight now runs the shared FighterAI + SF4 nav + poise.
    public class LothricSpearKnightOriginal : ModNPC
    {
        public override string Texture => "tsorcRevamp/NPCs/Enemies/LothricSpearKnight";
        ...
        int lothricKnight; //Used for when there is a Lothric Knight nearby
        int lothricKnight2;
        ...
        public override void AI()
        {
            ...
            if (AI_State == State_Pursuing)
            {
                ...
                {   // This whole block of code only works before it encounters a lothric knight, works while its with the lothric knight, but doesn-t revert once there is no lothric knight
                    ...
                    /*for (int i = 0; i < Main.maxNPCs; i++)
                    {
                        if (Main.npc[i].type == ModContent.NPCType<LothricKnight>() && NPC.Distance(Main.npc[i].Center) < 600 && Math.Abs(NPC.Center.X - player.Center.X) < 190f)
                        {
                            AI_State = State_JumpThrust;
                            AI_Timer = 0;
                        }
                        else if (Main.npc[i].type == ModContent.NPCType<LothricKnight>() && NPC.Distance(Main.npc[i].Center) < 600)
                        {
                        }
            ...
            if (AI_State == State_Shielding)
            {
                ...
                if (AI_Timer_Shielding > 310 && AI_Timer_Attacking == 420 && Math.Abs(NPC.Center.X - player.Center.X) > 6.5f * 16 && Math.Abs(NPC.Center.X - player.Center.X) < 19f * 16 && Math.Abs(NPC.Center.Y - player.Center.Y) < 12f * 16 && standing_on_solid_tile && NPC.velocity.Y == 0 && Collision.CanHitLine(NPC.Center, 0, 0, Main.player[NPC.target].Center, 0, 0))
                {   // This whole block of code only works before it encounters a lothric knight, works while its with the lothric knight, but doesn-t revert once there is no lothric knight
                    // If you think you can fix it go ahead
                    for (int i = 0; i < Main.maxNPCs; i++)
                    {
            ...
            if (AI_State == State_Thrusting)
            {
                ...
                if (NPC.direction == 1)
                {
                    if (AI_Timer == 34)
                    {
                        ...
                        {
                            Projectile stab = Main.projectile[Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + new Vector2(44, -2), new Vector2(0, 0), ModContent.ProjectileType<Spearhead>(), lothricSmallDamage, 5, Main.myPlayer, NPC.whoAmI, 3)];
                            stab.timeLeft = 6;
                            stab.velocity.X = 5;
                        }
                    }
                ...
                else
                {
                    if (AI_Timer == 34)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            Projectile stab = Main.projectile[Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + new Vector2(-46, -2), new Vector2(0, 0), ModContent.ProjectileType<Spearhead>(), lothricSmallDamage, 5, Main.myPlayer, NPC.whoAmI, 3)];
                            stab.timeLeft = 6;
                            stab.velocity.X = -5;
                        }
                    }
            ...
            if (AI_State == State_JumpThrust)
            {
                ...
                if (AI_Timer == 82)
                {
                    ...
                    if (NPC.direction == 1)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            Projectile stab = Main.projectile[Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + new Vector2(28, +38), new Vector2(0, 0), ModContent.ProjectileType<Spearhead>(), lothricBigDamage, 5, Main.myPlayer, NPC.whoAmI, 0)];
                            stab.timeLeft = 2;
                        }
            ...
            if (AI_State == State_LungeThrust)
            {
                ...
                if (AI_Timer >= 30 && AI_Timer < 56 && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    if (NPC.direction == 1)
                    {
                        Projectile stab = Main.projectile[Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + new Vector2(80, -2), new Vector2(0, 0), ModContent.ProjectileType<Spearhead>(), lothricDamage, 5, Main.myPlayer, NPC.whoAmI, 0)];
                        stab.timeLeft = 2;
                    }
                    else
                    {
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            ...
            npcLoot.Add(ItemDropRule.ByCondition(tsorcRevamp.tsorcItemDropRuleConditions.CursedRule, ModContent.ItemType<RadiantLifegemItem>(), 3, 1, 2));
