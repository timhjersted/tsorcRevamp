...
namespace tsorcRevamp.NPCs.Enemies.SuperHardMode
{
    class OolacileKnight : ModNPC
    {
        ...
        public override void AI()  //  warrior ai
        {
            ...
            float enrage_acceleration = .05f;  //  faster when enraged, usually 2*acceleration
            ...
            float hop_power = 6; // how hard/high offensive hops are; usually 4
            ...
            #endregion
            //------------------------------------------------------------------
            #region melee movement and dust effect
            ...
            int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 54, npc.velocity.X, npc.velocity.Y, 200, color, 1f + comboDamage / 320);
            Main.dust[dust].noGravity = true;
            ...
            if (!oBored && shoot_and_walk && Main.netMode != 1 && !Main.player[npc.target].dead) // can generalize this section to moving+projectile code 
            {
                ...
                if (Main.netMode != 1)
                {
                    ...
                    if (customspawn1 == 0)
                    {
                        if ((customspawn1 < 1) && Main.rand.Next(650) == 1)
                        {
                            int Spawned = NPC.NewNPC((int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), ModContent.NPCType<OolacileDemon>(), 0);
                    ...
                    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                    {
                        ...
                        if (Main.rand.Next(200) == 1)
                        {
                            ...
                            if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                            {
                                ...
                                int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, earthTridentDamage, 0f, Main.myPlayer);
        ...
        public void DealtNPC(double damage, Player player)
        {
            ...
            if (comboDamage > 1550)
            {
                ...
                {
                    int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 54, 0, 0, 100, color, 2f);
                }
