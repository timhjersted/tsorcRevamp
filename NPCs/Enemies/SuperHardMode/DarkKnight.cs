...
namespace tsorcRevamp.NPCs.Enemies.SuperHardMode
{
	class DarkKnight : ModNPC
	{
		...
		public override void AI()  //  warrior ai
		{
			...
			int boredom_time = 40; // time until it stops targeting player if blocked etc, 60 for anything but chaos ele, 20 for chaos ele
			...
			float acceleration = 0.4f;  //  how fast it can speed up
			float top_speed = 1.4f;  //  max walking speed, also affects jump length
			float braking_power = .4f;  //  %of speed that can be shed every tick when above max walking speed
			...
			float enrage_top_speed = 1f;  //  faster when enraged, usually 2*top_speed
			...
			int projectile_id = ModContent.ProjectileType<Projectiles.Enemy.EnemySpellAbyssStormWave>(); // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
			float projectile_velocity = 15; // initial velocity? 11 for Skeleton Archers, 9 for Goblin Archers, bombs have fixed speed & direction atm
			...
			if (is_archer)
			{
				int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 27, npc.velocity.X, npc.velocity.Y, 150, Color.Green, 1f);
					...
					if (npc.ai[2] > 0f) // if aiming: adjust aim and fire if needed
					{
						...
						{ // firing:
							...
							if (Main.netMode != 1)  //  is server
								Projectile.NewProjectile(npc_center.X, npc_center.Y, npc_to_target_x, npc_to_target_y, projectile_id, stormWaveDamage, 0f, Main.myPlayer);
			...
			if (Main.netMode != 1 && can_teleport && npc.ai[3] >= (float)boredom_time) // is server & chaos ele & bored
			{
				...
				int tp_radius = 60; // radius around target(upper left corner) in blocks to teleport into
				int tp_counter = 0;
				bool flag7 = false;
				...
				while (!flag7) // loop always ran full 100 time before I added "flag7 = true;" below
				{
					...
					{ // (tp_x_target,m) is block under its feet I think
						if ((m < target_y_blockpos - 30 || m > target_y_blockpos + 30 || tp_x_target < target_x_blockpos - 30 || tp_x_target > target_x_blockpos + 30) && (m < y_blockpos - 1 || m > y_blockpos + 1 || tp_x_target < x_blockpos - 1 || tp_x_target > x_blockpos + 1) && Main.tile[tp_x_target, m].active())
