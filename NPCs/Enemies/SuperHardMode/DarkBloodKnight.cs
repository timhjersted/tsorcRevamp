...
namespace tsorcRevamp.NPCs.Enemies.SuperHardMode
{
	class DarkBloodKnight : ModNPC
	{
		...
		public override void AI()  //  warrior ai
		{
			...
			int boredom_time = 80; // time until it stops targeting player if blocked etc, 60 for anything but chaos ele, 20 for chaos ele
			...
			float acceleration = 1f;  //  how fast it can speed up
			...
			int shot_rate = 110;  //  rate at which archers/bombers fire; 70 for skeleton archer, 180 for goblin archer, 450 for clown; atm must be an even # or won't fire at shot_rate/2
			//int fuse_time = 300;  //  fuse time on bombs, 300 for clown bombs
			int projectile_id = ModContent.ProjectileType<Projectiles.Enemy.EnemyBlackFireWave>(); // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
			float projectile_velocity = 13; // initial velocity? 11 for Skeleton Archers, 9 for Goblin Archers, bombs have fixed speed & direction atm
			...
			if (is_archer)
			{
				...
				int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 6, npc.velocity.X, npc.velocity.Y, 150, Color.Red, 1f);
					...
					if (npc.ai[2] > 0f) // if aiming: adjust aim and fire if needed
					{
						...
						{ // firing:
							...
							if (Main.netMode != 1)  //  is server
								Projectile.NewProjectile(npc_center.X, npc_center.Y, npc_to_target_x, npc_to_target_y, projectile_id, blackFireDamage, 0f, Main.myPlayer);
