...
namespace tsorcRevamp.NPCs.Enemies.SuperHardMode
{
    class SlograII : ModNPC
    {
        ...
        public override void AI()  //  warrior ai
        {
            ...
            int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 18, npc.velocity.X, npc.velocity.Y, 200, color, 1f + comboDamage / 350);
            Main.dust[dust].noGravity = true;
        ...
        public void DealtNPC(double damage, Player player)
        {
            ...
            if (comboDamage > 860)
            {
                ...
                {
                    int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 4, 0, 0, 100, color, 2f);
                }
                for (int num36 = 0; num36 < 20; num36++)
                {
                    int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 18, 0, 0, 100, color, 2f);
                }
