...
namespace tsorcRevamp.NPCs.Enemies.SuperHardMode
{
    class Tetsujin : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            NPC.lifeMax = 3400; //was 35k a long time ago
            NPC.scale = 1f;
        ...
        public override void AI()
        {
            if (Main.netMode != 1)
            {
                if (Main.GameUpdateCount % 240 == 0 && (Main.player[NPC.target].DistanceSQ(NPC.Center) < 887100))
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, projVel, ModContent.ProjectileType<EnemyRedLaser>(), 20, 0, Main.myPlayer, NPC.target, NPC.whoAmI);
                }
                    ...
                    if (Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
                    {
                        ...
                        if (breath)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                int num54 = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center.X, NPC.Center.Y, (float)((Math.Cos(rotation) * 25) * -1), (float)((Math.Sin(rotation) * 25) * -1), ModContent.ProjectileType<DragonsBreath>(), breathDamage, 0f, Main.myPlayer);
                        ...
                        if (Main.GameUpdateCount % 60 < 15 && Main.GameUpdateCount % 3 == 0 && (Main.player[NPC.target].DistanceSQ(NPC.Center) < 887100) && Collision.CanHit(NPC, Main.player[NPC.target]))
                        {
                            ...
                            if (((speedX < 0f) && (NPC.velocity.X < 0f)) || ((speedX > 0f) && (NPC.velocity.X > 0f)))
                            {
                                ...
                                speedX *= num51;
                                speedY *= num51;
                                int type = ModContent.ProjectileType<TetsujinLaser>();//44;//0x37; //14;
                                if (Main.netMode != NetmodeID.MultiplayerClient)
                                {
        ...
        public override void OnKill()
        {
            if (!Main.dedServ)
            {
                Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("Tetsujin Gore 1").Type, 0.9f);
                Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("Tetsujin Gore 2").Type, 0.9f);
        ...
        {
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<TeslaBolt>(), 1, 200, 230));
