...
namespace tsorcRevamp.NPCs.Enemies.SuperHardMode
{
	class Abysswalker : ModNPC
	{
		...
		public override void AI()  //  warrior ai
		{
			...
			//int projectile_damage = meteorDamage;  //  projectile dmg: 35 for Skeleton Archer, 11 for Goblin Archer
					...
					if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
					{
						if (Main.rand.Next(35) == 1)
						{
							...
							if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
							{
								...
								int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, poisonBallDamage, 0f, Main.myPlayer);
						...
						if (Main.rand.Next(140) == 1)
						{
							...
							if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
							{
								...
								speedX *= num51;
								speedY *= num51;
								int type = ModContent.ProjectileType<Projectiles.Enemy.EnemySpellAbyssStormBall>();//44;//0x37; //14;
								int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, stormBallDamage, 0f, Main.myPlayer);
