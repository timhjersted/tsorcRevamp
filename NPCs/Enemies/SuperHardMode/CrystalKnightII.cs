...
{
	class CrystalKnightII : ModNPC
	{
		public override void SetStaticDefaults()
		{			
			DisplayName.SetDefault("Dark Crystal Knight");
		}
		public override void SetDefaults()
		{
			...
			npc.value = 1000;
			banner = npc.type;
			bannerItem = ModContent.ItemType<Banners.CrystalKnightIIBanner>();
		}

		int crystalBoltDamage = 43;
		int archerBoltDamage = 85;

		public override void ScaleExpertStats(int numPlayers, float bossLifeScale)
		{
			npc.lifeMax = (int)(npc.lifeMax / 2);
			npc.damage = (int)(npc.damage / 2);
			crystalBoltDamage = (int)(crystalBoltDamage / 2);
		...
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			...
			if (tsorcRevampWorld.SuperHardMode && !Meteor && !Jungle && !Dungeon && !Corruption && Ocean && Main.rand.Next(100) == 1) return 1;


			if (tsorcRevampWorld.SuperHardMode && Main.bloodMoon && !Meteor && !Jungle && !Dungeon && !Corruption && !AboveEarth && Ocean && Main.rand.Next(30) == 1) return 1;
		...
		public override void AI()  //  warrior ai
		{
			...
			bool is_archer = true; // stops and shoots when target sighted; skel archer & gob archer are the archers
			bool can_pass_doors = false;  //  can open or break doors; c. bunny, crab, clown, skel archer, gob archer, & chaos elemental cannot
			bool shoot_and_walk = false;  //  can shoot while walking like clown; uses ai[2] so cannot be used with is_archer or can_pass_doors
			...
			float acceleration = 0.6f;  //  how fast it can speed up
			...
			int shot_rate = 80;  //  rate at which archers/bombers fire; 70 for skeleton archer, 180 for goblin archer, 450 for clown; atm must be an even # or won't fire at shot_rate/2
			//int fuse_time = 300;  //  fuse time on bombs, 300 for clown bombs
			int projectile_id = ModContent.ProjectileType<Projectiles.Enemy.EnemyCrystalKnightBoltII>(); // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
			...
			if (is_archer)
			{
				...
				int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 15, npc.velocity.X, npc.velocity.Y, 150, Color.Blue, 1f);
				...
				else // not confused
				{
					...
					if (npc.ai[2] > 0f) // if aiming: adjust aim and fire if needed
					{
						...
						//this section has been altered so that there's no errors in aiming since the projectile is a straight shot
						...
						{ // firing:
							...
							npc_to_target_x += (float)Main.rand.Next(-40, 40); //  targeting error: 40 pix=2.5 blocks
							npc_to_target_y += (float)Main.rand.Next(-40, 40); //  targeting error: 40 pix=2.5 blocks
							...
							if (Main.netMode != 1)  //  is server
								Projectile.NewProjectile(npc_center.X, npc_center.Y, npc_to_target_x, npc_to_target_y, projectile_id, crystalBoltDamage, 0f, Main.myPlayer);
					...
					{ // not aiming & no jump/fall & fire/reload ctr is 0 & target is alive and LOS to target: start aiming
						...
						npc_to_target_x += (float)Main.rand.Next(-41, 41);
						npc_to_target_y += (float)Main.rand.Next(-41, 41); //was -41 and 41
			...
			if (Main.netMode != 1 && can_teleport && npc.ai[3] >= (float)boredom_time) // is server & chaos ele & bored
			{
				...
				int tp_counter = 0;
				bool flag7 = false;
				if (Math.Abs(npc.position.X - Main.player[npc.target].position.X) + Math.Abs(npc.position.Y - Main.player[npc.target].position.Y) > 100000f)
				...
				while (!flag7) // loop always ran full 100 time before I added "flag7 = true;" below
				{
					...
					{ // (tp_x_target,m) is block under its feet I think
						if ((m < target_y_blockpos - 10 || m > target_y_blockpos + 10 || tp_x_target < target_x_blockpos - 10 || tp_x_target > target_x_blockpos + 10) && (m < y_blockpos - 1 || m > y_blockpos + 1 || tp_x_target < x_blockpos - 1 || tp_x_target > x_blockpos + 1) && Main.tile[tp_x_target, m].active())
						{ // over 30 blocks distant from player & over 1 block distant from old position & tile active(to avoid surface? want to tp onto a block?)
						...
						} // END over 30 blocks distant from player...
