...
namespace tsorcRevamp.NPCs.Enemies
{
	public class GhostOfTheForgottenWarrior : ModNPC
	{
		...
		public override void AI()  //  warrior ai
		{
			...
			float acceleration = .04f;  //  how fast it can speed up
			...
			if (can_teleport)  //  chaos elemental type teleporter
			{
				if (npc.ai[3] == -120f)  //  boredom goes negative? I think this makes disappear/arrival effects after it just teleported
				{
					...
					for (int j = 0; j < 10; j++) // make 20 dusts at current position
					{
						int num9 = Dust.NewDust(npc.position, npc.width, npc.height, 43, 0, 0, 190, default(Color), 1f);
					...
					for (int k = 0; k < 10; k++) // more dust effects at old position
					{
						int num10 = Dust.NewDust(npc.oldPos[2], npc.width, npc.height, 43, 0, 0, 190, default(Color), 1f);
			...
			if (is_archer)
			{
				...
				int dust = Dust.NewDust(new Vector2((float)npc.position.X, (float)npc.position.Y), npc.width, npc.height, 54, npc.velocity.X, npc.velocity.Y, 200, color, 1f);
					...
					if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
					{
						if (Main.rand.Next(100) == 1)
						{
							...
							if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
							{
								...
								int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, 20, 0f, Main.myPlayer);
			...
			if (Main.netMode != 1 && can_teleport && npc.ai[3] >= (float)boredom_time) // is server & chaos ele & bored
			{
				...
				{ // far away from target; follow to the ends of the earth
					tp_counter = 100;
				...
				while (!flag7) // loop always ran full 100 time before I added "flag7 = true" below
				{
					...
					{ // (tp_x_target,m) is block under its feet I think
						...
						{ // over 20 blocks distant from player & over 1 block distant from old position & tile active(to avoid surface? want to tp onto a block?)
