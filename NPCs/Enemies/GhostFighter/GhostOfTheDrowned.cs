...
namespace tsorcRevamp.NPCs.Enemies.GhostFighter
{
    class GhostOfTheDrowned : ModNPC, IStaggerable
    {
        ...
        public int spearStabDamage = 32;
        public int bubbleDamage = 24;
            ...
            if (AI_State == State_Thrusting)
            {
                ...
                if (NPC.direction == 1)
                {
                    if (AI_Timer == 34)
                    {
                        ...
                        {
                            Projectile stab = Main.projectile[Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + new Vector2(44, -2), new Vector2(0, 0), ModContent.ProjectileType<Spearhead>(), EnemyDamage.Projectile(spearStabDamage), 5, Main.myPlayer, NPC.whoAmI, 3, 2)];
                            stab.timeLeft = 6;
                            stab.velocity.X = 5;
                        }
                    }
                ...
                else
                {
                    if (AI_Timer == 34)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            Projectile stab = Main.projectile[Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + new Vector2(-46, -2), new Vector2(0, 0), ModContent.ProjectileType<Spearhead>(), EnemyDamage.Projectile(spearStabDamage), 5, Main.myPlayer, NPC.whoAmI, 3, 2)];
                            stab.timeLeft = 6;
                            stab.velocity.X = -5;
                        }
                    }
                ...
                if (AI_Timer % bubbleDelay == 0 && AI_Timer >= 90) //Every 8 frames
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        if (NPC.Distance(player.Center) > 160f)
                        {
                            ...
                            Projectile bolt = Main.projectile[Projectile.NewProjectile(NPC.GetSource_FromThis(), shootPos, boltVelocity, ModContent.ProjectileType<GhostBubbleBolt>(), EnemyDamage.Projectile(bubbleDamage), 5, Main.myPlayer, boltDrift)];
                            bolt.friendly = false;
                            bolt.hostile = true;
                        }
                        else
                        {
                            Projectile bubble = Main.projectile[Projectile.NewProjectile(NPC.GetSource_FromThis(), shootPos, new Vector2(NPC.direction * Main.rand.NextFloat(6, 12), Main.rand.NextFloat(-2f, 2f)), ProjectileID.Bubble, EnemyDamage.Projectile(bubbleDamage), 5, Main.myPlayer, NPC.whoAmI)];
