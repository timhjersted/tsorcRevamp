...
namespace tsorcRevamp.NPCs.Enemies
{
    class UndeadCaster : ModNPC
    {
        ...
        public override float SpawnChance(NPC.Spawner spawner)
        {
            ...
            if (!Main.hardMode && p.ZoneRockLayerHeight && !spawner.Player.ZoneDungeon && !spawner.Player.ZoneJungle && Main.dayTime) return 0.0285f;
            if (!Main.hardMode && p.ZoneRockLayerHeight && !spawner.Player.ZoneDungeon && !spawner.Player.ZoneJungle && !Main.dayTime) return 0.05f;
            if (!Main.hardMode && Sky(p) && !Main.dayTime) return 0.025f;
            if (!Main.hardMode && Sky(p) && Main.dayTime) return 0.05f;
            if (!Main.hardMode && spawner.Player.ZoneSnow) return 0.1f;
            if (!Main.hardMode && (spawner.Player.ZoneRockLayerHeight && !(!spawner.Player.ZoneDungeon && spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson || spawner.Player.ZoneJungle)) && !Main.dayTime) return 0.033f;
