...
namespace tsorcRevamp.NPCs.Enemies
{
    public class HollowWarrior : ModNPC //Don't look at the code, it's muy malo. Look at Lothric Spear Knight for a better example code management-wise
    {
        ...
        public int smallSlashDamage = 9;
        public int bigSlashDamage = 14;
        ...
        public override float SpawnChance(NPC.Spawner spawner)
        {
            ...
            if (!Main.hardMode && spawner.SpawnTileType == TileID.GreenDungeonBrick && !spawner.waterTile) return 0.07f;
            if (!spawner.waterTile && (Main.tile[spawner.SpawnTileX, spawner.SpawnTileY].WallType == WallID.GreenDungeonSlabUnsafe || Main.tile[spawner.SpawnTileX, spawner.SpawnTileY].WallType == WallID.GreenDungeonUnsafe) && !Main.hardMode && !tsorcRevampWorld.SuperHardMode) return 0.05f;
            if (Main.hardMode && spawner.ZoneLihzhardTemple) return 0.1f;
            if (Main.hardMode && p.ZoneNormalCaverns && !spawner.waterTile) return 0.01f;
            if (Main.hardMode && p.ZoneDesert && p.ZoneOverworldHeight && !spawner.waterTile) return 0.025f;
            if (Main.hardMode && p.ZoneUndergroundDesert && !spawner.waterTile) return 0.035f;
            if (Main.hardMode && spawner.SpawnTileType == TileID.BlueDungeonBrick && !spawner.waterTile) return 0.09f;
            if (Main.hardMode && spawner.SpawnTileType == TileID.TungstenBrick && !spawner.waterTile) return 0.08f;

            if (tsorcRevampWorld.SuperHardMode && !(Ocean || spawner.Player.ZoneJungle || spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson || spawner.Player.ZoneUnderworldHeight)) return 0.115f;
            if (tsorcRevampWorld.SuperHardMode && spawner.Player.ZoneOverworldHeight && !(Ocean || spawner.Player.ZoneJungle || spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson || spawner.Player.ZoneSkyHeight)) return 0.13f;
            if (tsorcRevampWorld.SuperHardMode && spawner.Player.ZoneDesert && !Ocean) return 0.15f;
            if (tsorcRevampWorld.SuperHardMode && spawner.Player.ZoneDungeon && !spawner.Player.ZoneUnderworldHeight) return 0.08f; //.08% is 4.28%


            if (Main.expertMode && Main.bloodMoon && spawner.Player.ZoneOverworldHeight && !(Ocean || spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson || spawner.Player.ZoneSkyHeight)) return chance = 0.04f;

            if (Main.expertMode && Main.bloodMoon && !(Ocean || spawner.Player.ZoneJungle || spawner.Player.ZoneHallow || spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson || spawner.Player.ZoneSnow || spawner.Player.ZoneSkyHeight || spawner.Player.ZoneUnderworldHeight)) return chance = 0.02f;

            // not sure why these were added since base game is expert mode
            if (((!Main.expertMode && (NPC.downedBoss1 || NPC.downedBoss2)) || Main.expertMode) && spawner.Player.ZoneOverworldHeight && Main.dayTime && !(Ocean || spawner.Player.ZoneJungle || spawner.Player.ZoneHallow || spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson || spawner.Player.ZoneSnow || spawner.Player.ZoneBeach || spawner.Player.ZoneSkyHeight)) return chance = 0.025f;
            if (((!Main.expertMode && (NPC.downedBoss1 || NPC.downedBoss2)) || Main.expertMode) && spawner.Player.ZoneOverworldHeight && !Main.dayTime && !(spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson)) return chance = 0.05f;

            if (((!Main.expertMode && (NPC.downedBoss1 || NPC.downedBoss2)) || Main.expertMode) && (spawner.Player.ZoneDirtLayerHeight || spawner.Player.ZoneRockLayerHeight) && Main.dayTime && !(Ocean || spawner.Player.ZoneJungle || spawner.Player.ZoneHallow || spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson || spawner.Player.ZoneSnow)) return chance = 0.035f;
            if (((!Main.expertMode && (NPC.downedBoss1 || NPC.downedBoss2)) || Main.expertMode) && (spawner.Player.ZoneDirtLayerHeight || spawner.Player.ZoneRockLayerHeight) && !Main.dayTime && !(Ocean || spawner.Player.ZoneJungle || spawner.Player.ZoneHallow || spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson || spawner.Player.ZoneSnow)) return chance = 0.05f;


            if ((!Main.expertMode && !(spawner.Player.ZoneUnderworldHeight || spawner.Player.ZoneJungle || spawner.Player.ZoneCorrupt || spawner.Player.ZoneCrimson) && (NPC.downedBoss1 || NPC.downedBoss2)) || Main.expertMode) return chance = 0.02f;
