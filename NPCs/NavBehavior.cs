...
namespace tsorcRevamp.NPCs
{
    ...
    public static class NavBehavior
    {
        ...
        public static PursuitState UpdateState(
            NPC npc, tsorcRevampGlobalNPC globalNPC, Player player, bool hasLos, bool madeProgress, float aggroRange)
        {
            ...
            globalNPC.FsmTick = (int)Main.GameUpdateCount;
            globalNPC.FsmEntryState = globalNPC.PursuitState;
            globalNPC.FsmEntryDisengage = globalNPC.DisengageTimer;
            globalNPC.FsmBranch = "-";

            // Record the spawn anchor once, the first time we ever tick.
            if (!globalNPC.PatrolAnchorSet && globalNPC.PatrolAnchorSource == PatrolAnchorSource.SpawnPoint)
            {
                globalNPC.PatrolAnchor = npc.Center;
                globalNPC.PatrolAnchorSet = true;
            }
            ...
            bool leashEnabled = globalNPC.PursuitState == PursuitState.Pursue
                && globalNPC.PursuitLeashRange > 0f && globalNPC.PursuitFallBehindTicks > 0;
            bool fallingBehind = leashEnabled
                && npc.Distance(player.Center) > globalNPC.PursuitLeashRange
                && !madeProgress;
            globalNPC.PursuitFallBehindTimer = fallingBehind ? globalNPC.PursuitFallBehindTimer + 1 : 0;
            ...
            if (leashEnabled && globalNPC.PursuitFallBehindTimer >= globalNPC.PursuitFallBehindTicks)
            {
                globalNPC.PursuitFallBehindTimer = 0;
                if (globalNPC.RemembersLastKnownPos)
                {
                ...
                npc.netUpdate = true;
                return FsmExit(globalNPC, "leash");
            }
            ...
            if (hasLos && globalNPC.PursuitState != PursuitState.Flee && globalNPC.GhostUnreachableWanderTimer <= 0)
            {
                ...
                if (globalNPC.PursuitState == PursuitState.Pursue || inAggro)
                {
                    ...
                    return FsmExit(globalNPC, "los-reaquire");
                }
            }

            switch (globalNPC.PursuitState)
            {
                case PursuitState.Pursue:
                    globalNPC.FsmBranch = madeProgress ? "pursue-progress" : "pursue-noprogress";
                    ...
                    if (globalNPC.DisengageTimer >= globalNPC.NavGiveUpTicks + (npc.whoAmI % 61))
                    {
                        if (globalNPC.RemembersLastKnownPos)
                        {
                            globalNPC.PursuitState = PursuitState.Search;
                ...
                case PursuitState.Search:
                    globalNPC.DisengageTimer++;
                    bool reachedLastKnown = npc.Distance(globalNPC.LastKnownPlayerPos) < ReachLastKnownPx;
                    globalNPC.FsmBranch = reachedLastKnown ? "search-arrived" : "search-tick";
                    if (reachedLastKnown || globalNPC.DisengageTimer >= SearchTimeoutTicks + (npc.whoAmI % 31))
                        EnterPatrol(npc, globalNPC);
                    break;

                case PursuitState.Patrol:
                    // Stay patrolling; re-aggro is handled by the hasLos branch above.
                    globalNPC.FsmBranch = "patrol-hold";
                    break;
                ...
                case PursuitState.Flee:
                    ...
                    globalNPC.FsmBranch = "flee-hold";
                    break;
            }

            return FsmExit(globalNPC, globalNPC.FsmBranch);
        }
        ...
        /// </summary>
        private static void StampMutation(tsorcRevampGlobalNPC globalNPC, string entry, string caller, int line)
        {
            globalNPC.FsmMutationTick = (int)Main.GameUpdateCount;
            globalNPC.FsmMutationBy = $"{entry}<-{caller}:{line}";
        }

        private static PursuitState FsmExit(tsorcRevampGlobalNPC globalNPC, string branch)
        {
            globalNPC.FsmBranch = branch;
            globalNPC.FsmExitState = globalNPC.PursuitState;
            globalNPC.FsmExitDisengage = globalNPC.DisengageTimer;
            return globalNPC.PursuitState;
        }
        ...
        /// </summary>
        public static void ForceDisengage(NPC npc, tsorcRevampGlobalNPC globalNPC,
        ...
        public static void EnterPatrol(NPC npc, tsorcRevampGlobalNPC globalNPC,
            [CallerMemberName] string caller = "?", [CallerLineNumber] int line = 0)
        {
            StampMutation(globalNPC, "EnterPatrol", caller, line);
            ...
            if (globalNPC.PatrolAnchorSource == PatrolAnchorSource.GiveUpLocation || !globalNPC.PatrolAnchorSet)
            {
                globalNPC.PatrolAnchor = npc.Center;
                globalNPC.PatrolAnchorSet = true;
            }
        ...
        /// <summary>
        /// Run one tick of Flee: walk away from wherever the flee started (globalNPC.FleeDirection, locked in when
        ...
        /// </summary>
        public static void RunFlee(NPC npc, tsorcRevampGlobalNPC globalNPC, float topSpeed, float acceleration)
        {
            globalNPC.FleeElapsedFrames++;
            float rampT = Math.Min(globalNPC.FleeElapsedFrames / FleeRampFrames, 1f);
            float fleeSpeed = topSpeed * (FleeSpeedMultStart + (FleeSpeedMultMax - FleeSpeedMultStart) * rampT);
            float traveledTiles = Math.Abs(npc.Center.X - globalNPC.FleeOriginX) / TileF;

            bool reachedSafety = traveledTiles >= FleeMaxTiles;
            bool blocked = reachedSafety || !StepAlong(npc, globalNPC.FleeDirection, fleeSpeed, acceleration, speedMult: 1f);
            if (blocked)
            {
                ...
                EnterPatrol(npc, globalNPC);
                RunPatrol(npc, globalNPC, topSpeed, acceleration);
            }
        }
        ...
        private static void RunWander(NPC npc, tsorcRevampGlobalNPC globalNPC, float topSpeed, float acceleration)
        {
            ...
            {
                globalNPC.PatrolTerrainRecoveryTimer--;
            }
            bool beyond = BeyondLeash(npc, globalNPC);
            if (beyond && globalNPC.PatrolTerrainRecoveryTimer <= 0)
            {
                globalNPC.PatrolDirection = AnchorDirection(npc, globalNPC);
            }
            ...
            if (!StepAlong(npc, globalNPC.PatrolDirection, topSpeed, acceleration))
            {
                globalNPC.PatrolDirection = -globalNPC.PatrolDirection; // turned at a gap/wall
            }
            else if (!beyond)
            {
                if (globalNPC.PatrolIdleTimer > 0)
        ...
        private static void RunTerrainWander(NPC npc, tsorcRevampGlobalNPC globalNPC, float topSpeed, float acceleration, bool beyond)
        {
            int direction = globalNPC.PatrolDirection;
            if (npc.velocity.Y != 0f)
            {
                globalNPC.PatrolTerrainAction = "air-commit";
                npc.direction = direction;
                npc.spriteDirection = direction;
                float jumpSpeed = Math.Max(topSpeed, globalNPC.MaxJumpBoost);
                npc.velocity.X = MathHelper.Lerp(npc.velocity.X, direction * jumpSpeed, 0.25f);
                return;
            }

            if (!StepAlongTerrain(npc, globalNPC, direction, topSpeed, acceleration))
            {
                globalNPC.PatrolDirection = -direction;
                if (beyond)
                {
                    ...
                    globalNPC.PatrolTerrainRecoveryTimer = TerrainLeashReplanTicks;
                    globalNPC.PatrolTerrainAction = "leash-replan";
                }
                else
                {
                    ...
                    globalNPC.PatrolIdleTimer = GetBlockedTurnCommitFrames(globalNPC, topSpeed);
                }
                return;
            }
        ...
        private static void ChooseNextWanderDirection(NPC npc, tsorcRevampGlobalNPC globalNPC)
        {
            if (globalNPC.PatrolTargetDirectionBias >= 0f && npc.HasValidTarget)
            {
        ...
        {
            if (globalNPC.PatrolWanderMaxTiles > 0)
            {
                int minTiles = Math.Max(Math.Max(1, globalNPC.PatrolWanderMinTiles), minimumTiles);
                int maxTiles = Math.Max(minTiles, globalNPC.PatrolWanderMaxTiles);
        ...
        private static int GetBlockedTurnCommitFrames(tsorcRevampGlobalNPC globalNPC, float topSpeed)
        {
            ...
            int recoveryMinimum = globalNPC.PatrolWanderMaxTiles > 0
                ? Math.Min(15, globalNPC.PatrolWanderMaxTiles)
                : 0;
        ...
        private static bool StepAlongTerrain(NPC npc, tsorcRevampGlobalNPC globalNPC, int direction, float topSpeed, float acceleration)
        {
            ...
            if (wall || !floor)
            {
                if (!TryStartTerrainJump(npc, globalNPC, direction, topSpeed))
                {
                    globalNPC.PatrolTerrainAction = wall ? "blocked-wall-no-landing" : "blocked-drop-no-landing";
                    Brake(npc);
                    return false;
                }
                return true;
            ...
            globalNPC.PatrolTerrainAction = floorDepth > 0 ? $"walk-down-{floorDepth}" : "walk-level";
        ...
        private static bool TryStartTerrainJump(NPC npc, tsorcRevampGlobalNPC globalNPC, int direction, float topSpeed)
        {
            ...
            for (int forwardTiles = 2; forwardTiles <= PatrolJumpSearchTiles; forwardTiles++)
            {
                ...
                foreach (int tileOffsetY in PatrolJumpLandingOffsets)
                {
                    ...
                    npc.direction = direction;
                    npc.spriteDirection = direction;
                    npc.velocity = new Vector2(direction * Math.Max(topSpeed, globalNPC.MaxJumpBoost), -globalNPC.MaxJumpPower);
                    globalNPC.PatrolTerrainAction = $"jump-{forwardTiles}t-land{tileOffsetY}";
                    npc.netUpdate = true;
                    return true;
                }
            }
        ...
        internal static string DescribeTerrainWander(NPC npc, tsorcRevampGlobalNPC globalNPC)
        {
            int direction = globalNPC.PatrolDirection == 0 ? 1 : globalNPC.PatrolDirection;
            int frontX = LeadingTileX(npc, direction);
            int feetY = (int)((npc.Bottom.Y - 1f) / TileF);
            int floorDepth = FloorDepthAhead(npc, direction);
            float fromAnchor = (npc.Center.X - globalNPC.PatrolAnchor.X) / TileF;
            return $" terrain={globalNPC.PatrolTerrainAction} front=({frontX},{feetY})"
                + $" wall={WallAhead(npc, direction)} drop={floorDepth}"
                + $" anchorDx={fromAnchor:F1}/{globalNPC.PatrolRange} replan={globalNPC.PatrolTerrainRecoveryTimer}";
        }
        ...
        private static int AnchorDirection(NPC npc, tsorcRevampGlobalNPC globalNPC)
            => globalNPC.PatrolAnchor.X >= npc.Center.X ? 1 : -1;

        private static bool BeyondLeash(NPC npc, tsorcRevampGlobalNPC globalNPC)
            => Math.Abs(npc.Center.X - globalNPC.PatrolAnchor.X) > globalNPC.PatrolRange * TileF;
        ...
        private static int FloorDepthAhead(NPC npc, int dir)
        {
            int frontX = LeadingTileX(npc, dir);
            int feetY = (int)((npc.Bottom.Y - 1f) / TileF);
            for (int depth = 0; depth <= CliffLookDownTiles; depth++)
                if (Standable(frontX, feetY + depth))
                {
                    return depth;
                }
