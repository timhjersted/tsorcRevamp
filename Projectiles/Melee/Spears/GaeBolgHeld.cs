...
{
    class GaeBolgHeld : ChargedBowHeld
    {
        ...
        protected override void Shoot()
        {
            ...
            if (player.altFunctionUse != 2)
            {
                ...
                Projectile.NewProjectile(player.GetSource_ItemUse(player.inventory[player.selectedItem]), Projectile.Center, velVector, ModContent.ProjectileType<GaeBolgThrown>(), damage, Projectile.knockBack, Projectile.owner, (charge >= 1) ? 1 : 0);
            ...
            else
            {
                ...
                Projectile.NewProjectile(player.GetSource_ItemUse(player.inventory[player.selectedItem]), Projectile.Center, velVector, ModContent.ProjectileType<GaeBolgPoke>(), maxDamage / 6, Projectile.knockBack, Projectile.owner);
            }
        }
        ...
        public override void PostDraw(Player player, Color lightColor)/* tModPorter Replace 'Main.player[Projectile.owner]' with 'player'. */
        {
            ...
            if (charge >= 1)
            {
                data = GameShaders.Armor.GetSecondaryShader((byte)GameShaders.Armor.GetShaderIdFromItemId(ItemID.StardustDye), Main.LocalPlayer);
            }
            else
            {
                data = GameShaders.Armor.GetSecondaryShader((byte)GameShaders.Armor.GetShaderIdFromItemId(ItemID.SkyBlueDye), Main.LocalPlayer);
            }
