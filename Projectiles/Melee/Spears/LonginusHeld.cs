...
{
    class LonginusHeld : ChargedBowHeld
    {
        ...
        protected override void Shoot()
        {
            ...
            if (player.altFunctionUse != 2)
            {
                ...
                Projectile.NewProjectile(player.GetSource_ItemUse(player.inventory[player.selectedItem]), Projectile.Center, velVector, ModContent.ProjectileType<LonginusThrown>(), damage, Projectile.knockBack, Projectile.owner, (charge >= 1) ? 1 : 0);
            ...
            else
            {
                ...
                Projectile.NewProjectile(player.GetSource_ItemUse(player.inventory[player.selectedItem]), Projectile.Center, velVector, ModContent.ProjectileType<LonginusPoke>(), maxDamage / 6, Projectile.knockBack, Projectile.owner);
            }
        }
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            if (charge >= 1)
            {
                data = GameShaders.Armor.GetSecondaryShader((byte)GameShaders.Armor.GetShaderIdFromItemId(ItemID.RedDye), Main.LocalPlayer);
            }
            else
            {
                data = GameShaders.Armor.GetSecondaryShader((byte)GameShaders.Armor.GetShaderIdFromItemId(ItemID.RedDye), Main.LocalPlayer);
