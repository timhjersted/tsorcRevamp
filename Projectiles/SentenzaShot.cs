    ...
    class SentenzaShot : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            if (Main.netMode != NetmodeID.Server)
            {
                ...
                if (option == 0)
                {
                    Terraria.Audio.SoundEngine.PlaySound(new Terraria.Audio.SoundStyle("tsorcRevamp/Sounds/Custom/RicochetUno") with { Volume = 0.45f, PitchVariance = .3f }, Projectile.Center);
                }
                else if (option == 1)
                {
                    Terraria.Audio.SoundEngine.PlaySound(new Terraria.Audio.SoundStyle("tsorcRevamp/Sounds/Custom/RicochetDos") with { Volume = 0.45f, PitchVariance = .3f }, Projectile.Center);
                }
                else if (option == 2)
                {
                    Terraria.Audio.SoundEngine.PlaySound(new Terraria.Audio.SoundStyle("tsorcRevamp/Sounds/Custom/RicochetTres") with { Volume = 0.45f, PitchVariance = .3f }, Projectile.Center);
                }
            }
        }
    }
