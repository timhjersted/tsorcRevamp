...
namespace tsorcRevamp.Projectiles.Enemy
{
    class EnemySmokebomb : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int g = 0; g < 2; g++)
            {
                int goreIndex = if (!Main.dedServ)
                {
                    Gore.NewGore(Projectile.GetSource_Death(), new Vector2(Projectile.position.X + (float)(Projectile.width / 2) - 24f, Projectile.position.Y + (float)(Projectile.height / 2) - 24f), default(Vector2), Main.rand.Next(61, 64), .8f);
