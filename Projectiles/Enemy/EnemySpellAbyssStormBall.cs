			...
			if (Main.myPlayer == projectile.owner && projectile.ai[0] == 0f)
			{
				...
				else
				{
					if (projectile.ai[0] == 0f)
					{
						...
						if (num63 == 0f)
						{
							Projectile.NewProjectile(projectile.position.X + (float)(projectile.width * +3f), projectile.position.Y + (float)(projectile.height * +5.5f), 0, 0, ModContent.ProjectileType<SpellUltimateExplosion>(), projectile.damage, 8f, projectile.owner);
