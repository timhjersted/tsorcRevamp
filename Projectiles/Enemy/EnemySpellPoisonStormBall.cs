    ...
    public class EnemySpellPoisonStormBall : ModProjectile
    {
			...
			if (Main.myPlayer == projectile.owner && projectile.ai[0] == 0f)
			{
				...
				else
				{
					if (projectile.ai[0] == 0f)
					{
						...
						if (num63 == 0f)
						{
							Projectile.NewProjectile(projectile.position.X + (float)(projectile.width * +3f), projectile.position.Y + (float)(projectile.height * +5.5f), 0, 0, ModContent.ProjectileType<Projectiles.Enemy.SpellUltimateExplosion>(), (int)(this.projectile.damage), 8f, projectile.owner);
		...
		public override void Kill(int timeLeft)
		{
			...
			projectile.timeLeft = 0;
			{
				...
				if (projectile.owner == Main.myPlayer) Projectile.NewProjectile(new Vector2(projectile.position.X + (float)(projectile.width * + 3f), projectile.position.Y + (float)(projectile.height * + 5.5f)), new Vector2(0, 0), ModContent.ProjectileType<Projectiles.Enemy.EnemySpellPoisonStorm>(), (int)(this.projectile.damage), 8f, projectile.owner);
