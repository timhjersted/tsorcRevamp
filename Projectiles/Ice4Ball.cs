...
namespace tsorcRevamp.Projectiles {
    class Ice4Ball : ModProjectile {
		...
		public override void SetDefaults() {
			...
			{
				//For each, check if it's modded. If so, check if it's the Ice4Ball.
				if (Main.projectile[i].ModProjectile != null && Main.projectile[i].ModProjectile is Projectiles.Ice4Ball)
				{
					//Cast it to an Ice4Ball so we can check if it's currently being channeled, and make sure it's still active
		...
		public override void Kill(int timeLeft) {
			...
			Projectile.timeLeft = 0;
			{
				...
				Projectile.NewProjectile(Projectile.position.X + (float)(Projectile.width), Projectile.position.Y + (float)(Projectile.height), 0, 5, ModContent.ProjectileType<Ice4Icicle>(), (int)(Projectile.damage), 3f, Projectile.owner);
				Projectile.NewProjectile(Projectile.position.X + (float)(Projectile.width * 5), Projectile.position.Y + (float)(Projectile.height * 4), 0, 5, ModContent.ProjectileType<Ice4Icicle>(), (int)(Projectile.damage), 3f, Projectile.owner);
				Projectile.NewProjectile(Projectile.position.X + (float)(Projectile.width * -3), Projectile.position.Y + (float)(Projectile.height * 7), 0, 5, ModContent.ProjectileType<Ice4Icicle>(), (int)(Projectile.damage), 3f, Projectile.owner);
				Projectile.NewProjectile(Projectile.position.X + (float)(Projectile.width), Projectile.position.Y + (float)(Projectile.height * 10), 0, 5, ModContent.ProjectileType<Ice4Icicle>(), (int)(Projectile.damage), 3f, Projectile.owner);
