...
namespace tsorcRevamp.Projectiles
{
    class Meteor : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 30; i++)
            {
                int dust = Dust.NewDust(new Vector2(Projectile.position.X - (float)(Projectile.width / 2), Projectile.position.Y - (float)(Projectile.height / 2)), Projectile.width, Projectile.height, 293, Main.rand.Next(-10, 10) + Projectile.velocity.X, Main.rand.Next(-10, 10) + Projectile.velocity.Y, 160, default, 3f);
                dust = Dust.NewDust(new Vector2(Projectile.position.X - (float)(Projectile.width / 2), Projectile.position.Y - (float)(Projectile.height / 2)), Projectile.width, Projectile.height, 293, Main.rand.Next(-10, 10) + Projectile.velocity.X, Main.rand.Next(-10, 10) + Projectile.velocity.Y, 160, default, 3f);
                Main.dust[dust].noGravity = true;
