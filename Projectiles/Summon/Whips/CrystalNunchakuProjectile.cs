...
namespace tsorcRevamp.Projectiles.Summon.Whips
{
	public class CrystalNunchakuProjectile : ModProjectile
	{
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            Vector2 WhipTip = new Vector2(10, 32) * Main.player[Main.myPlayer].whipRangeMultiplier * Projectile.WhipSettings.RangeMultiplier * player.GetModPlayer<tsorcRevampPlayer>().WhipCritHitboxSize;
            List<Vector2> points = Projectile.WhipPointsForCollision;
        ...
        {
            if (!target.HasBuff(ModContent.BuffType<CrystalNunchakuDebuff>()))
			{
                target.AddBuff(ModContent.BuffType<CrystalNunchakuDebuff>(), (int)(15 * 60 * Main.player[Projectile.owner].GetModPlayer<tsorcRevampPlayer>().SummonTagDuration));
            }
		...
		private void DrawLine(List<Vector2> list)
		{
			...
			for (int i = 0; i < list.Count - 1; i++)
			{
				...
				Color color = Lighting.GetColor(element.ToTileCoordinates(), Color.BlueViolet);
