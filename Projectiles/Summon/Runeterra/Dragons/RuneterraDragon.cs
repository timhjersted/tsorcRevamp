...
namespace tsorcRevamp.Projectiles.Summon.Runeterra.Dragons
{
    ...
    public abstract class RuneterraDragon : ModProjectile
    {
        ...
        public void StartAltSequence()
        {
            ...
            CometZone = Projectile.NewProjectileDirect(Projectile.GetSource_None(), Main.MouseWorld, Vector2.Zero, ModContent.ProjectileType<CenterOfTheUniverseCometZone>(), (int)BaseOriginalDamage * 2, player.GetTotalKnockback(DamageClass.Summon).ApplyTo(player.HeldItem.knockBack * 10), player.whoAmI); //this damage only applies to the shockwave after the explosion and hits once
        }
