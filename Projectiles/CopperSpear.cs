...
{
    class CopperSpear : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (!pOwner.frozen)
            {
                ...
                { //when initially thrown
                    moveFactor = 1.29f; //move forward (2.4% of projectile scaled sprite size)
                    Projectile.netUpdate = true;
                }
                if (pOwner.itemAnimation < pOwner.itemAnimationMax / 2)
                { //after x animation frames, return
                    moveFactor -= 1.18f; //2.2% of projctile scaled sprite size
                }
                else
                { //extend spear
                    moveFactor += 1.29f; //(2.4% of projectile scaled sprite size)
                }
