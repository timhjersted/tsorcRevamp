...
namespace tsorcRevamp.Utilities
{
    public class WikiExporterCommand : ModCommand
    {
        ...
        private sealed class ArmorSetGroup
        {
            public string Key { get; init; }
            public string WikiName { get; init; }
            public List<ModItem> Heads { get; } = new List<ModItem>();
            public List<ModItem> Bodies { get; } = new List<ModItem>();
            public List<ModItem> Legs { get; } = new List<ModItem>();
        }
        ...
        public override void Action(CommandCaller caller, string input, string[] args)
        {
            try
            {
                ...
                string armorSetsFile = workspaceExists
                    ? Path.Combine(workspacePath, "armor_sets_data.lua")
                    : Path.Combine(savePath, "tsorc_armor_sets_data.lua");
                ...
                List<ArmorSetGroup> armorSets = BuildArmorSets(modItems);
                HashSet<int> armorSetPieceTypes = armorSets.SelectMany(set => set.Heads.Concat(set.Bodies).Concat(set.Legs)).Select(item => item.Type).ToHashSet();
                List<ModItem> wikiItems = modItems.Where(item => !armorSetPieceTypes.Contains(item.Type)).ToList();
                ...
                EnsureUniqueWikiPageNames(wikiItems, wikiNPCs, armorSets);
                ...
                ExportArmorSets(armorSetsFile, armorSets);
                ExportNPCs(npcsFile, wikiNPCs);
                ExportPagesXml(xmlFile, wikiItems, wikiNPCs, armorSets);
        ...
        private void ExportArmorSets(string filePath, IEnumerable<ArmorSetGroup> armorSets)
        {
            var builder = new StringBuilder();
            builder.AppendLine("-- This file is auto-generated by the /exportwikidata command. Do not edit by hand.");
            builder.AppendLine("return {");

            foreach (ArmorSetGroup armorSet in armorSets)
            {
                builder.AppendLine($"    [\"{EscapeLua(armorSet.Key)}\"] = {{");
                builder.AppendLine($"        name = \"{EscapeLua(armorSet.WikiName)}\",");
                builder.AppendLine($"        wikiName = \"{EscapeLua(armorSet.WikiName)}\",");
                AppendArmorSetPieces(builder, "heads", armorSet.Heads);
                AppendArmorSetPieces(builder, "bodies", armorSet.Bodies);
                AppendArmorSetPieces(builder, "legs", armorSet.Legs);
                builder.AppendLine("    },");
            }
        ...
        private static void AppendArmorSetPieces(StringBuilder builder, string slot, IEnumerable<ModItem> pieces)
        {
            builder.AppendLine($"        {slot} = {{");
            foreach (ModItem piece in pieces.OrderBy(item => item.Name, StringComparer.Ordinal))
            {
                builder.AppendLine($"            \"{EscapeLua(piece.Name)}\",");
            }
            builder.AppendLine("        },");
        }
        ...
        private static void EnsureUniqueWikiPageNames(IEnumerable<ModItem> modItems, IEnumerable<ModNPC> modNPCs, IEnumerable<ArmorSetGroup> armorSets)
        {
            List<string> duplicateNames = modItems.Select(item => GetWikiPageName(ContentSamples.ItemsByType[item.Type].Name, item.Name))
                ...
                .Concat(armorSets.Select(set => set.WikiName))
        ...
        private static List<ArmorSetGroup> BuildArmorSets(IEnumerable<ModItem> modItems)
        {
            List<ModItem> armorItems = modItems.Where(item => IsArmorPiece(ContentSamples.ItemsByType[item.Type])).ToList();
            List<ModItem> heads = armorItems.Where(item => ContentSamples.ItemsByType[item.Type].headSlot != -1).ToList();
            List<ModItem> bodies = armorItems.Where(item => ContentSamples.ItemsByType[item.Type].bodySlot != -1).ToList();
            List<ModItem> legs = armorItems.Where(item => ContentSamples.ItemsByType[item.Type].legSlot != -1).ToList();
            var setsByWikiName = new Dictionary<string, ArmorSetGroup>(StringComparer.Ordinal);

            foreach (ModItem definingPiece in armorItems)
            {
                Item definingItem = ContentSamples.ItemsByType[definingPiece.Type];
                if (definingItem.headSlot != -1)
                {
                    foreach (ModItem body in bodies)
                    {
                        foreach (ModItem legsPiece in legs)
                        {
                            TryAddArmorSet(setsByWikiName, definingPiece, definingPiece, body, legsPiece);
                        }
                    }
                }
                else if (definingItem.bodySlot != -1)
                {
                    foreach (ModItem head in heads)
                    {
                        foreach (ModItem legsPiece in legs)
                        {
                            TryAddArmorSet(setsByWikiName, definingPiece, head, definingPiece, legsPiece);
                        }
                    }
                }
                else if (definingItem.legSlot != -1)
                {
                    foreach (ModItem head in heads)
                    {
                        foreach (ModItem body in bodies)
                        {
                            TryAddArmorSet(setsByWikiName, definingPiece, head, body, definingPiece);
                        }
                    }
                }
            }

            return setsByWikiName.Values.OrderBy(set => set.WikiName, StringComparer.Ordinal).ToList();
        }

        private static bool IsArmorPiece(Item item)
        {
            return item.headSlot != -1 || item.bodySlot != -1 || item.legSlot != -1;
        }

        private static void TryAddArmorSet(IDictionary<string, ArmorSetGroup> setsByWikiName, ModItem definingPiece, ModItem head, ModItem body, ModItem legs)
        {
            Item headItem = ContentSamples.ItemsByType[head.Type];
            Item bodyItem = ContentSamples.ItemsByType[body.Type];
            Item legsItem = ContentSamples.ItemsByType[legs.Type];
            if (!definingPiece.IsArmorSet(headItem, bodyItem, legsItem))
            {
                return;
            }

            string wikiName = GetArmorSetWikiName(definingPiece);
            if (!setsByWikiName.TryGetValue(wikiName, out ArmorSetGroup armorSet))
            {
                armorSet = new ArmorSetGroup
                {
                    Key = $"ArmorSet_{definingPiece.Name}",
                    WikiName = wikiName
                };
                setsByWikiName[wikiName] = armorSet;
            }

            AddArmorPiece(armorSet.Heads, head);
            AddArmorPiece(armorSet.Bodies, body);
            AddArmorPiece(armorSet.Legs, legs);
        }

        private static void AddArmorPiece(ICollection<ModItem> pieces, ModItem item)
        {
            if (!pieces.Any(piece => piece.Type == item.Type))
            {
                pieces.Add(item);
            }
        }

        private static string GetArmorSetWikiName(ModItem definingPiece)
        {
            string name = ContentSamples.ItemsByType[definingPiece.Type].Name;
            if (string.IsNullOrEmpty(name))
            {
                name = definingPiece.Name;
            }

            string[] pieceSuffixes =
            {
                " Plate Armor", " Breastplate", " Chestplate", " Waistcloth", " Hairstyle", " Hair Style",
                " Armor", " Helmet", " Leggings", " Greaves", " Bottoms", " Pants", " Shirt", " Tunic",
                " Helmet", " Mask", " Hood", " Hat", " Robe", " Mail", " Top", " Cloak", " Crown", " Gi"
            };

            foreach (string suffix in pieceSuffixes)
            {
                if (name.EndsWith(suffix, StringComparison.OrdinalIgnoreCase))
                {
                    string baseName = name.Substring(0, name.Length - suffix.Length).Trim();
                    if (!string.IsNullOrWhiteSpace(baseName))
                    {
                        return $"{baseName} Armor";
                    }
                }
            }

            return $"{name} Armor Set";
        }
        ...
        private void ExportPagesXml(string filePath, IEnumerable<ModItem> modItems, IEnumerable<ModNPC> modNPCs, IEnumerable<ArmorSetGroup> armorSets)
        {
            var builder = new StringBuilder();
            ...
            foreach (ModItem modItem in modItems)
            {
                ...
                pageContent.AppendLine($"{{{{The Story of Red Cloud/Item|id={modItem.Name}}}}}");
            ...
            foreach (ArmorSetGroup armorSet in armorSets)
            {
                string pageTitle = $"The Story of Red Cloud/Armor/{armorSet.WikiName}";
                var pageContent = new StringBuilder();
                pageContent.AppendLine("{{mod sub-page}}");
                pageContent.AppendLine($"{{{{The Story of Red Cloud/Armor|id={armorSet.Key}}}}}");
            ...
            foreach (ModNPC modNPC in modNPCs)
            {
                ...
                var pageContent = new StringBuilder();
                pageContent.AppendLine("{{mod sub-page}}");
                pageContent.AppendLine($"{{{{The Story of Red Cloud/NPC|id={modNPC.Name}}}}}");
