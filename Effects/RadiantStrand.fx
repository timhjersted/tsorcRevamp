...
float4 MainPS(VertexShaderOutput input) : COLOR0
{
    ...
    float4 effectColor = noiseColor * pow(intensity, 1) * 2.4;
    effectColor -= float4(1, 1, 1, 1) * (pow(intensity, 4.0) * sampleIntensity);
    
    if (intensity > 1.5)
    {
        effectColor = float4(1, 1, 1, (5 * intensity) - 7.5);
    }
    if (intensity > 1.7)
    {
        effectColor = float4(1, 1, 1, 1) * pow(intensity, 10.0);
    }
...
technique RadiantStrand
{
    pass RadiantStrandPass
    {
